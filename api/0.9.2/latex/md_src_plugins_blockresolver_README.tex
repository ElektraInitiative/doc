...
The {\ttfamily blockresolver} can be used to only resolve a tagged block inside a configuration file.\hypertarget{md_src_plugins_blockresolver_README_autotoc_md66}{}\doxysubsection{Implementation Details}\label{md_src_plugins_blockresolver_README_autotoc_md66}
{\ttfamily blockresolver} extracts the requested block from the configurations file and writes it into a temporary file. Afterwards Elektra will only work on the temporary file until kdb\+Set is called. On kdb\+Set the contents of the temporary file will be merged with parts outside of the requested block from the original file.\hypertarget{md_src_plugins_blockresolver_README_autotoc_md67}{}\doxysection{Usage}\label{md_src_plugins_blockresolver_README_autotoc_md67}
...
\end{DoxyItemize}\hypertarget{md_src_plugins_blockresolver_README_autotoc_md68}{}\doxysection{Limitations}\label{md_src_plugins_blockresolver_README_autotoc_md68}
Currently the identifier must be unique.\hypertarget{md_src_plugins_blockresolver_README_autotoc_md69}{}\doxysection{Example}\label{md_src_plugins_blockresolver_README_autotoc_md69}
...
\DoxyCodeLine{echo '[section1]'             >> \$(kdb get system/tests/blockfile)}
\DoxyCodeLine{echo 'key1=val1'            >> \$(kdb get system/tests/blockfile)}
\DoxyCodeLine{echo '[section2]'             >> \$(kdb get system/tests/blockfile)}
\DoxyCodeLine{echo 'key2=val2'            >> \$(kdb get system/tests/blockfile)}
...
\DoxyCodeLine{echo 'text'                   >> \$(kdb get system/tests/blockfile)}
\DoxyCodeLine{}
\DoxyCodeLine{sudo kdb mount -\/R blockresolver \$(kdb get system/tests/blockfile) system/tests/blockresolver -\/c identifier=">>> block config" ini}
\DoxyCodeLine{}
...
\DoxyCodeLine{kdb export system/tests/blockresolver ini}
\DoxyCodeLine{\#> [section1]}
\DoxyCodeLine{\#> key1=val1}
\DoxyCodeLine{\#> [section2]}
\DoxyCodeLine{\#> key2=val2}
...
\DoxyCodeLine{kdb set system/tests/blockresolver/section1/key12 val12}
\DoxyCodeLine{}
\DoxyCodeLine{cat \$(kdb get system/tests/blockfile)}
\DoxyCodeLine{\#> text}
\DoxyCodeLine{\#> more text}

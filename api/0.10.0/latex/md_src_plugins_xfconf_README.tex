...
You can refer to the \href{https://docs.xfce.org/xfce/xfconf/start}{\texttt{ official XFCE documentation of xfconf}} to learn more about it.\hypertarget{md_src_plugins_xfconf_README_autotoc_md614}{}\doxysection{Xfconf Terminology}\label{md_src_plugins_xfconf_README_autotoc_md614}
\hypertarget{md_src_plugins_xfconf_README_autotoc_md615}{}\doxysubsection{Property}\label{md_src_plugins_xfconf_README_autotoc_md615}
A property in xfconf is the same as a key in libelektra i.\+e. it has a name and can hold a value. In contrast to libelektra, the value can be more complex i.\+e. it can be an array. In this case, the value of the property is mapped as multiple keys in libelektra using the array structure.\hypertarget{md_src_plugins_xfconf_README_autotoc_md616}{}\doxysubsection{Channel}\label{md_src_plugins_xfconf_README_autotoc_md616}
...
The list of all channels is stored in the {\ttfamily system\+:/elektra/modules/xfconf/channels} which is an array of all channel names. Channels cannot be created manually with libelektra. Instead, when mounting a channel (see \href{\#plugin-configuration}{\texttt{ Plugin Configuration}} how to specify a channel) and setting a key value below the mount, it will be automatically created.\hypertarget{md_src_plugins_xfconf_README_autotoc_md617}{}\doxysubsection{Locks}\label{md_src_plugins_xfconf_README_autotoc_md617}
Xfconf uses so-\/called locks to prevent users or applications to set properties to a new value. They can be referred as a type of constant. As of now, locks cannot be set or unset in xfconf which makes locked properties read-\/only forever.\hypertarget{md_src_plugins_xfconf_README_autotoc_md618}{}\doxysection{Dependencies}\label{md_src_plugins_xfconf_README_autotoc_md618}
...
Note that this plugin does not support mac\+OS since Xfce is not commonly used there, Mac Ports only provides a very old version ({\ttfamily 4.\+12}) and Brew does not provide Xfce at all.\hypertarget{md_src_plugins_xfconf_README_autotoc_md619}{}\doxysection{Usage}\label{md_src_plugins_xfconf_README_autotoc_md619}
The usage is identical to most storage plugins except that the channel option during mount must be defined in order to tell xfconf which channel to mount.\hypertarget{md_src_plugins_xfconf_README_autotoc_md620}{}\doxysection{Plugin Configuration}\label{md_src_plugins_xfconf_README_autotoc_md620}
The required {\ttfamily channel} configuration option is used to tell xfconf which channel to mount.\hypertarget{md_src_plugins_xfconf_README_autotoc_md621}{}\doxysection{Examples}\label{md_src_plugins_xfconf_README_autotoc_md621}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_xfconf_README_autotoc_md622}{}\doxysection{Limitations}\label{md_src_plugins_xfconf_README_autotoc_md622}

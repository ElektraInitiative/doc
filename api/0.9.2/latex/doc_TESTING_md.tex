\hypertarget{doc_TESTING_md_autotoc_md2519}{}\doxysection{Introduction}\label{doc_TESTING_md_autotoc_md2519}
Libraries need pervasive testing for continuous improvement. Any problem found and behavior described must be written down as test so that it is assured that no new regressions will be added.\hypertarget{doc_TESTING_md_autotoc_md2520}{}\doxysection{Running Tests}\label{doc_TESTING_md_autotoc_md2520}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md2521}{}\doxysection{Required Environment}\label{doc_TESTING_md_autotoc_md2521}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{Then change the permissions:}
\end{DoxyCode}
...
\end{DoxyEnumerate}\hypertarget{doc_TESTING_md_autotoc_md2522}{}\doxysection{Manual Testing}\label{doc_TESTING_md_autotoc_md2522}
...
Pay attention that sourcing depends on the operating system or rather the shell. For example on standard Free\+B\+SD 11.\+3 you have to execute {\ttfamily sh} in the root of the repository first. Then do {\itshape not} use the {\ttfamily source} command but the point {\ttfamily .} as explained above.\hypertarget{doc_TESTING_md_autotoc_md2523}{}\doxysection{Using debuggers}\label{doc_TESTING_md_autotoc_md2523}
...
\item {\ttfamily git}, {\ttfamily clang-\/format} (version 9), and \href{https://github.com/cheshirekow/cmake_format}{\texttt{ cmake-\/format}} to check formatting.
...
You can also use Docker to set up such an environment. There is \mbox{\hyperlink{doc_tutorials_run_all_tests_with_docker_md}{a tutorial}} that guides you through the necessary steps.\hypertarget{doc_TESTING_md_autotoc_md2525}{}\doxysection{Adding Tests}\label{doc_TESTING_md_autotoc_md2525}
...
By using {\ttfamily T\+E\+S\+T\+\_\+\+R\+E\+A\+D\+ME} in {\ttfamily add\+\_\+plugin} (also enabled by default), \href{https://master.libelektra.org/tests/shell/shell_recorder/tutorial_wrapper}{\texttt{ Markdown Shell Recorder}} are expected to be in the R\+E\+A\+D\+M\+E.\+md of the plugin.\hypertarget{doc_TESTING_md_autotoc_md2526}{}\doxysection{Conventions}\label{doc_TESTING_md_autotoc_md2526}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md2527}{}\doxysection{Strategy}\label{doc_TESTING_md_autotoc_md2527}
The testing must happen on every level of the software to achieve a maximum coverage with the available time. In the rest of the document we describe the different levels and where these tests are.\hypertarget{doc_TESTING_md_autotoc_md2528}{}\doxysubsection{C\+Framework}\label{doc_TESTING_md_autotoc_md2528}
This is basically a bunch of assertion macros and some output facilities. It is written in pure C and very lightweight.

It is located here.\hypertarget{doc_TESTING_md_autotoc_md2529}{}\doxysubsection{A\+B\+I Tests}\label{doc_TESTING_md_autotoc_md2529}
...
The tests are located here.\hypertarget{doc_TESTING_md_autotoc_md2530}{}\doxysubsection{C Unit Tests}\label{doc_TESTING_md_autotoc_md2530}
...
They are located here.\hypertarget{doc_TESTING_md_autotoc_md2531}{}\doxysubsubsection{Internal Functions}\label{doc_TESTING_md_autotoc_md2531}
...
get exported. Functions not starting with this prefix are internal only and therefore not visible in the test cases. Test internal functionality by including the corresponding C file.\hypertarget{doc_TESTING_md_autotoc_md2532}{}\doxysubsection{Module Tests}\label{doc_TESTING_md_autotoc_md2532}
...
Use the C\+Make macro {\ttfamily add\+\_\+plugintest} for adding these tests.\hypertarget{doc_TESTING_md_autotoc_md2533}{}\doxysubsection{C++ Unit Tests}\label{doc_TESTING_md_autotoc_md2533}
...
Use the C\+Make macro {\ttfamily add\+\_\+gtest} for adding these tests.\hypertarget{doc_TESTING_md_autotoc_md2534}{}\doxysubsection{Script Tests}\label{doc_TESTING_md_autotoc_md2534}
...
See here.\hypertarget{doc_TESTING_md_autotoc_md2535}{}\doxysubsection{Shell Recorder}\label{doc_TESTING_md_autotoc_md2535}
The more elegant way to specify script tests are via the so called Shell Recorder using Markdown Syntax.

See here.\hypertarget{doc_TESTING_md_autotoc_md2536}{}\doxysubsection{Fuzz Testing}\label{doc_TESTING_md_autotoc_md2536}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\string~e/scripts/configure-\/debian -\/DCMAKE\_C\_COMPILER=/usr/src/afl/afl-\/2.52b/afl-\/gcc -\/DCMAKE\_CXX\_COMPILER=/usr/src/afl/afl-\/2.52b/afl-\/g++ \string~e}
\end{DoxyCode}
...
\DoxyCodeLine{cp \string~e/src/plugins/ini/ini/* testcase\_dir}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{LD\_LIBRARY\_PATH=`pwd`/lib /usr/src/afl/afl-\/2.52b/afl-\/fuzz -\/i testcase\_dir -\/o findings\_dir bin/kdb import user/tests ini}
\end{DoxyCode}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md2537}{}\doxysubsection{A\+S\+AN}\label{doc_TESTING_md_autotoc_md2537}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md2538}{}\doxysubsubsection{mac\+OS}\label{doc_TESTING_md_autotoc_md2538}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md2539}{}\doxysubsection{C\+B\+MC}\label{doc_TESTING_md_autotoc_md2539}
For bounded model checking tests, see {\ttfamily scripts/cbmc}.\hypertarget{doc_TESTING_md_autotoc_md2540}{}\doxysubsection{Static Code Checkers}\label{doc_TESTING_md_autotoc_md2540}
There is a number of static code checkers available for all kind of programming languages. The following section show how the most common ones can be used with {\ttfamily libelektra}.\hypertarget{doc_TESTING_md_autotoc_md2541}{}\doxysubsubsection{Cppcheck}\label{doc_TESTING_md_autotoc_md2541}
...
Now you can view the html report by opening {\ttfamily index.\+html} in the specified folder to get an overview of the issues found in the whole project.\hypertarget{doc_TESTING_md_autotoc_md2542}{}\doxysubsubsection{O\+C\+Lint}\label{doc_TESTING_md_autotoc_md2542}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{cd ..}
\DoxyCodeLine{oclint -\/p build -\/no-\/analytics -\/enable-\/global-\/analysis -\/enable-\/clang-\/static-\/analyzer src/plugins/ini/*.c}
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md2543}{}\doxysubsubsection{scan-\/build}\label{doc_TESTING_md_autotoc_md2543}
...
Alternatively, you can also open the {\ttfamily index.\+html} file in the aforementioned folder, but using the tool the report is enriched with further information.\hypertarget{doc_TESTING_md_autotoc_md2544}{}\doxysubsubsection{Sonar\+Lint}\label{doc_TESTING_md_autotoc_md2544}
\href{http://www.sonarlint.org/}{\texttt{ Sonar\+Lint}} is a static code checker primarily intended for Java. It is usually used by installing the corresponding plugin for the used I\+DE, then there is no further configuration required.\hypertarget{doc_TESTING_md_autotoc_md2545}{}\doxysubsection{Randoop}\label{doc_TESTING_md_autotoc_md2545}
For using the unit test generator randoop with the jna bindings, see {\ttfamily scripts/randoop/randoop}.\hypertarget{doc_TESTING_md_autotoc_md2546}{}\doxysubsection{Code Coverage}\label{doc_TESTING_md_autotoc_md2546}
Run\+:
...
The H\+T\+ML files can be found in the build directory in the folder {\ttfamily coverage}.\hypertarget{doc_TESTING_md_autotoc_md2547}{}\doxysection{See Also}\label{doc_TESTING_md_autotoc_md2547}

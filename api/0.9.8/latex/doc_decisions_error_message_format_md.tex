...
\Hypertarget{doc_decisions_error_message_format_md_md_doc_decisions_error_message_format}%
 \hypertarget{doc_decisions_error_message_format_md_autotoc_md1808}{}\doxysection{Problem}\label{doc_decisions_error_message_format_md_autotoc_md1808}
Too verbose error message Currently for every error, 9 lines are shown in which most of them are not relevant to end users/administrators. One goal is to reduce the verbosity of such messages and let users/administrators see only information they need.\hypertarget{doc_decisions_error_message_format_md_autotoc_md1809}{}\doxysection{Constraints}\label{doc_decisions_error_message_format_md_autotoc_md1809}

\begin{DoxyItemize}
\item Supporting multiple programming languages
\item Plugin System
\item Error Code should be preserved
\end{DoxyItemize}\hypertarget{doc_decisions_error_message_format_md_autotoc_md1810}{}\doxysection{Assumptions}\label{doc_decisions_error_message_format_md_autotoc_md1810}
\hypertarget{doc_decisions_error_message_format_md_autotoc_md1811}{}\doxysection{Considered Alternatives}\label{doc_decisions_error_message_format_md_autotoc_md1811}
...
\end{DoxyItemize}\hypertarget{doc_decisions_error_message_format_md_autotoc_md1812}{}\doxysection{Decision}\label{doc_decisions_error_message_format_md_autotoc_md1812}
...
To avoid losing information, the user can use the command line argument {\ttfamily -\/v} (verbose) to show {\ttfamily Mountpoint}, {\ttfamily Configfile} in addition to the current error message. Furthermore a developer can use the command line argument {\ttfamily -\/d} (debug) to show {\ttfamily At} for debugging purposes.\hypertarget{doc_decisions_error_message_format_md_autotoc_md1813}{}\doxysection{Rationale}\label{doc_decisions_error_message_format_md_autotoc_md1813}
The new error message is much more succinct which gives end users more relevant information. Furthermore the solution approach still holds all necessary information if requested by users.\hypertarget{doc_decisions_error_message_format_md_autotoc_md1814}{}\doxysection{Implications}\label{doc_decisions_error_message_format_md_autotoc_md1814}
{\ttfamily Description} will be incorporated into {\ttfamily Reason} whereas the {\ttfamily Module} will be incorporated into the general sentence starting the error message.\hypertarget{doc_decisions_error_message_format_md_autotoc_md1815}{}\doxysection{Related Decisions}\label{doc_decisions_error_message_format_md_autotoc_md1815}
...
\end{DoxyItemize}\hypertarget{doc_decisions_error_message_format_md_autotoc_md1816}{}\doxysection{Notes}\label{doc_decisions_error_message_format_md_autotoc_md1816}

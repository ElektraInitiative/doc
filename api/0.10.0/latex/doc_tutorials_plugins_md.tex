...
\Hypertarget{doc_tutorials_plugins_md_md_doc_tutorials_plugins}%
 This file serves as a tutorial on how to write a plugin.\hypertarget{doc_tutorials_plugins_md_autotoc_md4511}{}\doxysection{Types of Plugins}\label{doc_tutorials_plugins_md_autotoc_md4511}
...
This tutorial mostly uses storage plugins as example but also explains differences to filter plugins.\hypertarget{doc_tutorials_plugins_md_autotoc_md4512}{}\doxysection{Basics}\label{doc_tutorials_plugins_md_autotoc_md4512}
First, there are a few basic points to understand about Elektra plugins. This first section will explain the basic layout of a plugin and what various methods exists within one.\hypertarget{doc_tutorials_plugins_md_autotoc_md4513}{}\doxysubsection{The Interface}\label{doc_tutorials_plugins_md_autotoc_md4513}
...
After these two steps your plugin is ready to be compiled, installed and mounted for the first time. Have a look at \mbox{\hyperlink{doc_tutorials_mount_md}{How-\/\+To\+: kdb mount}}\hypertarget{doc_tutorials_plugins_md_autotoc_md4514}{}\doxysubsubsection{C++ Based Plugins}\label{doc_tutorials_plugins_md_autotoc_md4514}
If you want to use C++ instead of C for plugin development you can use \`{}copy-\/template\`{} to create a plugin based on \`{}cpptemplate\`{}. For example, to create a new plugin called {\ttfamily pluginbaby} use the command\+:
...
\end{DoxyCode}
\hypertarget{doc_tutorials_plugins_md_autotoc_md4515}{}\doxysection{Contract}\label{doc_tutorials_plugins_md_autotoc_md4515}
...
To avoid this situation, plugins export a so-\/called {\itshape contract}. In this contract the plugin states how nicely it will behave and what other plugins can depend on.\hypertarget{doc_tutorials_plugins_md_autotoc_md4516}{}\doxysubsection{Writing a Contract}\label{doc_tutorials_plugins_md_autotoc_md4516}
...
\end{DoxyItemize}\hypertarget{doc_tutorials_plugins_md_autotoc_md4517}{}\doxysubsection{Content of $<$tt$>$\+README.\+md$<$/tt$>$}\label{doc_tutorials_plugins_md_autotoc_md4517}
...
\end{DoxyCode}
\hypertarget{doc_tutorials_plugins_md_autotoc_md4518}{}\doxysection{Including $<$tt$>$readme\+\_\+pluginname.\+c$<$/tt$>$}\label{doc_tutorials_plugins_md_autotoc_md4518}
...
The {\ttfamily elektra\+Plugin\+Contract()} is a method implemented by the plugin developer containing the parts of the contract not specified in {\ttfamily README.\+md}. An example of this function (taken from the \`{}yajl\`{} plugin)\+:
...
\end{DoxyCode}
\hypertarget{doc_tutorials_plugins_md_autotoc_md4519}{}\doxysection{CMake}\label{doc_tutorials_plugins_md_autotoc_md4519}
...
If your plugin makes use of \mbox{\hyperlink{doc_tutorials_compilation-variants_md}{compilation variants}} you should also read the information there.\hypertarget{doc_tutorials_plugins_md_autotoc_md4520}{}\doxysection{Coding}\label{doc_tutorials_plugins_md_autotoc_md4520}
This section will focus on an overview of the kind of code you would use to develop a plugin. It gives examples from real plugins and should serve as a rough guide on how to write a storage plugin that can read and write configuration data into an Elektra {\ttfamily Key\+Set}.\hypertarget{doc_tutorials_plugins_md_autotoc_md4521}{}\doxysubsection{$<$tt$>$elektra\+Plugin\+Get$<$/tt$>$}\label{doc_tutorials_plugins_md_autotoc_md4521}
...
\DoxyCodeLine{        ssize\_t ksSize = \mbox{\hyperlink{group__keyset_ga7474ad6b0a0fa969dbdf267ba5770eee}{ksGetSize}} (conf);}
\DoxyCodeLine{}
\DoxyCodeLine{        \textcolor{keywordflow}{for} (elektraCursor it = 0; it < ksSize; ++it)}
\DoxyCodeLine{        \{}
\DoxyCodeLine{                cur = \mbox{\hyperlink{group__keyset_gab3fb5e067c672d9fd60a4022b2ae9dd1}{ksAtCursor}} (conf, it);}
...
For further information see \href{https://doc.libelektra.org/api/latest/html/group__plugin.html}{\texttt{ the API documentation}}.\hypertarget{doc_tutorials_plugins_md_autotoc_md4527}{}\doxysubsection{$<$tt$>$elektra\+Plugin\+Get\+Global\+Key\+Set$<$/tt$>$}\label{doc_tutorials_plugins_md_autotoc_md4527}
...
To improve performance, the {\ttfamily cache} plugin also caches parts of the global keyset. If your plugin uses non-\/cacheable data, you don\textquotesingle{}t have to do anything special. However, if you want your plugin\textquotesingle{}s keys to be cached you should put them below {\ttfamily system\+:/elektra/cached} (to avoid collisions, use e.\+g. {\ttfamily system\+:/elektra/cached/$<$plugin$>$} for {\ttfamily $<$plugin$>$}). The {\ttfamily cache} plugin also caches keys below {\ttfamily system\+:/elektra/cache}, but those are reserved for use by the plugin itself.\hypertarget{doc_tutorials_plugins_md_autotoc_md4528}{}\doxysection{Note on Direct Method Calls via External Integrations}\label{doc_tutorials_plugins_md_autotoc_md4528}
Some applications want to call Elektra methods directly via native access. A {\ttfamily Key\+Set} is a data structure over which functions can iterate. If you want to start again from to first element, you have to explicitly call {\ttfamily rewind()} to set the internal pointer to the start. Any plugin expects the passed {\ttfamily Key\+Set} to be {\bfseries{rewinded}}.\hypertarget{doc_tutorials_plugins_md_autotoc_md4529}{}\doxysection{Memory Leaks}\label{doc_tutorials_plugins_md_autotoc_md4529}
If you experience memory leaks you may use {\ttfamily valgrind} in order to locate them. If you have analyzed your code, and you know that your code does not contain memory leaks continue reading\+:
...
\begin{DoxyItemize}
\item Update the plugin contract within the plugins {\ttfamily README.\+md} by appending {\ttfamily memleak} to {\ttfamily info/status} if not already done
\item If the {\ttfamily CMake\+Lists.\+txt} does not add the {\ttfamily MEMLEAK} label anywhere (just search for it in the file), append 
\begin{DoxyCode}{0}
\DoxyCodeLine{if (ADDTESTING\_PHASE)}
\DoxyCodeLine{  include (LibAddTest)}
\DoxyCodeLine{  add\_plugintest (replace\_with\_the\_actual\_plugin\_name MEMLEAK)}
\DoxyCodeLine{endif (ADDTESTING\_PHASE)}

\end{DoxyCode}
 to the end of the file and remove the {\ttfamily INSTALL\+\_\+\+TEST\+\_\+\+DATA} label from the {\ttfamily add\+\_\+plugin(...)} function.
\item Add the memleak rules to the {\ttfamily tests/valgrind.\+suppression} file.
\end{DoxyItemize}

The rules can be obtained through the jenkins pipeline (click on \char`\"{}details\char`\"{} next to the \char`\"{}continuous-\/integration/jenkins/pr-\/merge\char`\"{} Git\+Hub check) within the \char`\"{}\+Tests\char`\"{} tab at the top. There will be expandable items highlighted red. After expanding, they show a verbose output. Just look for the blocks in the following form\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{\{}
\DoxyCodeLine{   <insert\_a\_suppression\_name\_here>}
\DoxyCodeLine{   Memcheck:Leak}
\DoxyCodeLine{   match-\/leak-\/kinds: definite}
\DoxyCodeLine{   fun:malloc}
\DoxyCodeLine{   fun:malloc}
\DoxyCodeLine{   fun:resize\_scopes}
\DoxyCodeLine{   fun:dl\_open\_worker\_begin}
\DoxyCodeLine{   fun:\_dl\_catch\_exception}
\DoxyCodeLine{   fun:dl\_open\_worker}
\DoxyCodeLine{   fun:\_dl\_catch\_exception}
\DoxyCodeLine{   fun:\_dl\_open}
\DoxyCodeLine{   fun:dlopen\_doit}
\DoxyCodeLine{   fun:\_dl\_catch\_exception}
\DoxyCodeLine{   fun:\_dl\_catch\_error}
\DoxyCodeLine{   fun:\_dlerror\_run}
\DoxyCodeLine{   fun:dlopen\_implementation}
\DoxyCodeLine{   fun:dlopen@@GLIBC\_2.34}
\DoxyCodeLine{   fun:elektraModulesLoad}
\DoxyCodeLine{\}}

\end{DoxyCode}


and append them to the bottom of the {\ttfamily tests/valgrind.\+suppression} file and do not forget to give them a clear name.

After committing and pushing, the CI memleak errors should be suppressed.\hypertarget{doc_tutorials_plugins_md_autotoc_md4530}{}\doxysection{Further Readings}\label{doc_tutorials_plugins_md_autotoc_md4530}
Read more about\+:

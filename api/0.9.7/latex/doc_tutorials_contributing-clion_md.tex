...
\end{DoxyItemize}\hypertarget{doc_tutorials_contributing-clion_md_autotoc_md3516}{}\doxysection{Forking the Repository}\label{doc_tutorials_contributing-clion_md_autotoc_md3516}
...
To be able to make pull requests, you need a copy of this repository inside your Git\+Hub account. You can find a tutorial about how to do this \href{https://help.github.com/en/articles/fork-a-repo}{\texttt{ here}} (remember to sign into your account first). After this, you should see this copy in the list of your own repositories with a hint of it\textquotesingle{}s origin.\hypertarget{doc_tutorials_contributing-clion_md_autotoc_md3517}{}\doxysection{Getting the Code}\label{doc_tutorials_contributing-clion_md_autotoc_md3517}
To develop for libelektra, we now have to \char`\"{}download\char`\"{} your copy of its original repository. In git this process is called \char`\"{}cloning\char`\"{}. CLion has built-\/in Git support which we will use for this tutorial. Once you have opened CLion click on\+:

{\itshape Check out from Version Control} -\/-\/$>$ {\itshape Git}

and paste the URL of your forked libelektra-\/repository inside the {\itshape URL} field. Then choose the directory you want to store your project in. To be also able to directly push changes back to your repository, we\textquotesingle{}ll also configure CLion to be able to use your Git\+Hub account. Click on the \char`\"{}\+Log in to Git\+Hub\char`\"{} button and enter your Git\+Hub credentials and confirm. The last step here is to click \char`\"{}\+Clone\char`\"{} to download the project and open it in CLion.
...
With the project now locally available we can start developing.\hypertarget{doc_tutorials_contributing-clion_md_autotoc_md3518}{}\doxysection{Setting Up the Project}\label{doc_tutorials_contributing-clion_md_autotoc_md3518}
To import all of the project\textquotesingle{}s configuration, open CMake\+Lists.\+txt inside the project\textquotesingle{}s root directory and click on \char`\"{}\+Load CMake project\char`\"{} which will appear on the top right corner of the source view.

If you\textquotesingle{}ve cloned the project using a terminal. start CLion and once you see the main menu, click \char`\"{}\+Open\char`\"{} and select the CMake\+Lists.\+txt file inside the project\textquotesingle{}s root directory. This will import the project accordingly and populate you run configuration with some predefined values. In rare occurrences this won\textquotesingle{}t happen. If that is the case for you, simply restart CLion using\+:
...
which, in this case, starts 8 build jobs in parallel. For optimal performance this value should represent the number of available cores of your CPU + few extra jobs.
...
Make sure the selected \char`\"{}\+Scheme\char`\"{} is \char`\"{}\+Project\char`\"{}.\hypertarget{doc_tutorials_contributing-clion_md_autotoc_md3519}{}\doxysection{Development}\label{doc_tutorials_contributing-clion_md_autotoc_md3519}
...
\end{DoxyItemize}\hypertarget{doc_tutorials_contributing-clion_md_autotoc_md3520}{}\doxysection{Testing}\label{doc_tutorials_contributing-clion_md_autotoc_md3520}
...
For further information please read \mbox{\hyperlink{doc_TESTING_md}{this}}.\hypertarget{doc_tutorials_contributing-clion_md_autotoc_md3521}{}\doxysection{Commiting Your Changes}\label{doc_tutorials_contributing-clion_md_autotoc_md3521}
...
{\itshape VCS} -\/-\/$>$ {\itshape Git} -\/-\/$>$ {\itshape Push}
...
Please refer to \mbox{\hyperlink{doc_COMPILE_md}{this}} tutorial to fix the problem permanently.\hypertarget{doc_tutorials_contributing-clion_md_autotoc_md3525}{}\doxysection{Hints}\label{doc_tutorials_contributing-clion_md_autotoc_md3525}

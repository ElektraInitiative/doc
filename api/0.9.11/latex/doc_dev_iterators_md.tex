...
\Hypertarget{doc_dev_iterators_md_md_doc_dev_iterators}%
 The deprecated internal iterator are removed and replaced with external iteration.\hypertarget{doc_dev_iterators_md_autotoc_md2326}{}\doxysection{Interactions with users}\label{doc_dev_iterators_md_autotoc_md2326}
...
\end{DoxyCode}
\hypertarget{doc_dev_iterators_md_autotoc_md2327}{}\doxysection{The Architecture}\label{doc_dev_iterators_md_autotoc_md2327}
Every occurence of using the internal iterator has to be replaced with the external iterator (see \#4281). The functions of the internal iterator are listed in \#3171. There are also some uses of the internal iterator found in the SWIG bindings (Lua, Ruby, Python) that also need removal (see \#4279).\hypertarget{doc_dev_iterators_md_autotoc_md2328}{}\doxysubsection{Internal vs. external Iterators}\label{doc_dev_iterators_md_autotoc_md2328}
When the client (i.\+e. the programmer) controls the iteration, the iterator is called external iterator. Otherwise, when the iterator controls the iteration, it is called internal iterator. Due to the fact that external iterators are more flexible the internal iterators are the way to go.\hypertarget{doc_dev_iterators_md_autotoc_md2329}{}\doxysection{Examples}\label{doc_dev_iterators_md_autotoc_md2329}
...
For such scenarios, it is also important that you recalculate the size with {\ttfamily ks\+Get\+Size ()} within the loop-\/header or explicitely after changing the {\ttfamily Key\+Set}, e.\+g. by deleting a {\ttfamily Key}.
...
The following is a comprehension of how to use iterators in various languages.\hypertarget{doc_dev_iterators_md_autotoc_md2330}{}\doxysubsubsection{C}\label{doc_dev_iterators_md_autotoc_md2330}
...
\end{DoxyCode}
\hypertarget{doc_dev_iterators_md_autotoc_md2331}{}\doxysubsubsection{C++}\label{doc_dev_iterators_md_autotoc_md2331}
...
\end{DoxyCode}
\hypertarget{doc_dev_iterators_md_autotoc_md2332}{}\doxysubsubsection{Python}\label{doc_dev_iterators_md_autotoc_md2332}
...
\end{DoxyCode}
\hypertarget{doc_dev_iterators_md_autotoc_md2333}{}\doxysubsubsection{Lua}\label{doc_dev_iterators_md_autotoc_md2333}
...
\end{DoxyCode}
\hypertarget{doc_dev_iterators_md_autotoc_md2334}{}\doxysubsubsection{Ruby}\label{doc_dev_iterators_md_autotoc_md2334}

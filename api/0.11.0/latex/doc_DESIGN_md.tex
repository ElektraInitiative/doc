...
The C-\/\+API is suitable to be reimplemented, also in non-\/\+C-\/languages, like Rust. Elektra provides a full-\/blown architecture to support configuring systems, and the C-\/\+API is the core of this endeavour.\hypertarget{doc_DESIGN_md_autotoc_md2570}{}\doxysection{Data Structures}\label{doc_DESIGN_md_autotoc_md2570}
The {\ttfamily Key}, {\ttfamily Key\+Set} and {\ttfamily KDB} data structures are defined in {\ttfamily \mbox{\hyperlink{kdbprivate_8h}{kdbprivate.\+h}}} to allow ABI compatibility. This means, it is not possible to put one of Elektra’s data structures on the stack. You must use the memory management facilities mentioned in the next section.\hypertarget{doc_DESIGN_md_autotoc_md2571}{}\doxysection{Memory Management}\label{doc_DESIGN_md_autotoc_md2571}
...
gives the length of the value in bytes.\hypertarget{doc_DESIGN_md_autotoc_md2572}{}\doxysection{Variable Arguments}\label{doc_DESIGN_md_autotoc_md2572}
...
Alternatively pass a list as described in the documentation. The idea of these variable arguments is, that one function call can create any {\ttfamily Key\+Set}. For binding writers {\ttfamily key\+VNew} might be useful.\hypertarget{doc_DESIGN_md_autotoc_md2573}{}\doxysection{Off-\/by-\/one}\label{doc_DESIGN_md_autotoc_md2573}
...
The same is true for {\ttfamily elektra\+Str\+Len} which also already has the null byte included.\hypertarget{doc_DESIGN_md_autotoc_md2574}{}\doxysection{Minimal Set}\label{doc_DESIGN_md_autotoc_md2574}
...
Useful extensions are available in \mbox{\hyperlink{src_libs_README_md}{further libraries}}.\hypertarget{doc_DESIGN_md_autotoc_md2575}{}\doxysection{Return Values}\label{doc_DESIGN_md_autotoc_md2575}
...
Elektra does not set {\ttfamily errno}. If a function you call sets {\ttfamily errno}, make sure to set it back to the old value again.\hypertarget{doc_DESIGN_md_autotoc_md2577}{}\doxysection{Naming}\label{doc_DESIGN_md_autotoc_md2577}
...
Function names not belonging to one of the three classes use the prefix {\ttfamily elektra$\ast$}.\hypertarget{doc_DESIGN_md_autotoc_md2578}{}\doxysection{const}\label{doc_DESIGN_md_autotoc_md2578}
...
The reason behind this is, that the above functions – as their name suggest – only retrieve values. The returned value must not be modified directly.\hypertarget{doc_DESIGN_md_autotoc_md2579}{}\doxysection{Design Guidelines Checklist}\label{doc_DESIGN_md_autotoc_md2579}
On potential changes of the API/\+ABI as detected by the \href{https://build.libelektra.org/job/libelektra/job/master/}{\texttt{ {\ttfamily build server}}}, please make sure the API has been reviewed according to the following 2 checklists\+:\hypertarget{doc_DESIGN_md_autotoc_md2580}{}\doxysection{Checklist for overall API}\label{doc_DESIGN_md_autotoc_md2580}
\hypertarget{doc_DESIGN_md_autotoc_md2581}{}\doxysubsection{Consistency}\label{doc_DESIGN_md_autotoc_md2581}
...
\end{DoxyItemize}\hypertarget{doc_DESIGN_md_autotoc_md2582}{}\doxysubsection{Structural Clarity}\label{doc_DESIGN_md_autotoc_md2582}

\begin{DoxyItemize}
\item \mbox{[} \mbox{]} Functions with similar functionality have the same prefix
\end{DoxyItemize}\hypertarget{doc_DESIGN_md_autotoc_md2583}{}\doxysubsection{Compatibility}\label{doc_DESIGN_md_autotoc_md2583}

\begin{DoxyItemize}
\item \mbox{[} \mbox{]} All bindings have been updated to reflect the new API and work properly
\end{DoxyItemize}\hypertarget{doc_DESIGN_md_autotoc_md2584}{}\doxysubsection{Extensibility}\label{doc_DESIGN_md_autotoc_md2584}
...
\end{DoxyItemize}\hypertarget{doc_DESIGN_md_autotoc_md2585}{}\doxysection{Checklist for each function}\label{doc_DESIGN_md_autotoc_md2585}

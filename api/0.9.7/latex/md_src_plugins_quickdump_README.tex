...
The format is also useful for IPC and streaming, because of this it is used by the {\ttfamily specload} plugin.\hypertarget{md_src_plugins_quickdump_README_autotoc_md425}{}\doxysection{Format}\label{md_src_plugins_quickdump_README_autotoc_md425}
...
To distinguish between binary and string keys the (length of the) key value is prefixed with either a {\ttfamily b} or an {\ttfamily s}. Each metakey is prefixed with an {\ttfamily m}, unless we detect that the same metakey was already present on a previous key (e.\+g. through {\ttfamily key\+Copy\+Meta}). In this case the prefix {\ttfamily c} is used and instead of the metakey name and value, we write the name of the previous key and the metakey name.\hypertarget{md_src_plugins_quickdump_README_autotoc_md426}{}\doxysubsection{Variable Length Integer encoding}\label{md_src_plugins_quickdump_README_autotoc_md426}
...
Thanks to \href{https://github.com/stoklund/varint}{\texttt{ https\+://github.\+com/stoklund/varint}} for listing various integer encodings.\hypertarget{md_src_plugins_quickdump_README_autotoc_md427}{}\doxysection{Usage}\label{md_src_plugins_quickdump_README_autotoc_md427}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_quickdump_README_autotoc_md428}{}\doxysection{Dependencies}\label{md_src_plugins_quickdump_README_autotoc_md428}
None.\hypertarget{md_src_plugins_quickdump_README_autotoc_md429}{}\doxysection{Examples}\label{md_src_plugins_quickdump_README_autotoc_md429}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_quickdump_README_autotoc_md430}{}\doxysection{Limitations}\label{md_src_plugins_quickdump_README_autotoc_md430}
None. 
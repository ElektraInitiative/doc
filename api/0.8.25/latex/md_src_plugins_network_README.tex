...
Every key tagged with the metakey {\ttfamily check/ipaddr} will be checked using {\ttfamily getaddrinfo()}. If additionally the values {\ttfamily ipv4} or {\ttfamily ipv6} are supplied, the address family will be specified. If supplied only numerical hosts are allowed. If left empty, the plugin will resolve domain names and look if it is reachable (i.\+e. \char`\"{}localhost\char`\"{} should most likely work on any system) 
...
\Hypertarget{doc_TESTING_md_md_doc_TESTING}%
 \hypertarget{doc_TESTING_md_autotoc_md3685}{}\doxysection{Introduction}\label{doc_TESTING_md_autotoc_md3685}
Libraries need pervasive testing for continuous improvement. Any problem found and behavior described must be written down as test so that it is assured that no new regressions will be added.\hypertarget{doc_TESTING_md_autotoc_md3686}{}\doxysection{Running Tests}\label{doc_TESTING_md_autotoc_md3686}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md3687}{}\doxysection{Required Environment}\label{doc_TESTING_md_autotoc_md3687}
...
\end{DoxyEnumerate}\hypertarget{doc_TESTING_md_autotoc_md3688}{}\doxysection{Manual Testing}\label{doc_TESTING_md_autotoc_md3688}
...
Pay attention that sourcing depends on the operating system or rather the shell. For example on standard Free\+BSD 11.\+3 you have to execute {\ttfamily sh} in the root of the repository first. Then do {\itshape not} use the {\ttfamily source} command but the point {\ttfamily .} as explained above.\hypertarget{doc_TESTING_md_autotoc_md3689}{}\doxysection{Using debuggers}\label{doc_TESTING_md_autotoc_md3689}
...
You can also use Docker to set up such an environment. There is \mbox{\hyperlink{doc_tutorials_run_all_tests_with_docker_md}{a tutorial}} that guides you through the necessary steps.\hypertarget{doc_TESTING_md_autotoc_md3691}{}\doxysection{Adding Tests}\label{doc_TESTING_md_autotoc_md3691}
...
By using {\ttfamily TEST\+\_\+\+README} in {\ttfamily add\+\_\+plugin} (also enabled by default), \href{https://master.libelektra.org/tests/shell/shell_recorder/tutorial_wrapper}{\texttt{ Markdown Shell Recorder}} are expected to be in the README.\+md of the plugin.\hypertarget{doc_TESTING_md_autotoc_md3692}{}\doxysection{Conventions}\label{doc_TESTING_md_autotoc_md3692}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md3693}{}\doxysection{Strategy}\label{doc_TESTING_md_autotoc_md3693}
The testing must happen on every level of the software to achieve a maximum coverage with the available time. In the rest of the document we describe the different levels and where these tests are.\hypertarget{doc_TESTING_md_autotoc_md3694}{}\doxysubsection{CFramework}\label{doc_TESTING_md_autotoc_md3694}
This is basically a bunch of assertion macros and some output facilities. It is written in pure C and very lightweight.

It is located here.\hypertarget{doc_TESTING_md_autotoc_md3695}{}\doxysubsection{ABI Tests}\label{doc_TESTING_md_autotoc_md3695}
...
The tests are located here.\hypertarget{doc_TESTING_md_autotoc_md3696}{}\doxysubsection{C Unit Tests}\label{doc_TESTING_md_autotoc_md3696}
...
They are located here.\hypertarget{doc_TESTING_md_autotoc_md3697}{}\doxysubsubsection{Internal Functions}\label{doc_TESTING_md_autotoc_md3697}
...
get exported. Functions not starting with this prefix are internal only and therefore not visible in the test cases. Test internal functionality by including the corresponding C file.\hypertarget{doc_TESTING_md_autotoc_md3698}{}\doxysubsection{Module Tests}\label{doc_TESTING_md_autotoc_md3698}
...
Use the CMake macro {\ttfamily add\+\_\+plugintest} for adding these tests.\hypertarget{doc_TESTING_md_autotoc_md3699}{}\doxysubsection{C++ Unit Tests}\label{doc_TESTING_md_autotoc_md3699}
...
Use the CMake macro {\ttfamily add\+\_\+gtest} for adding these tests.\hypertarget{doc_TESTING_md_autotoc_md3700}{}\doxysubsection{Script Tests}\label{doc_TESTING_md_autotoc_md3700}
...
See here.\hypertarget{doc_TESTING_md_autotoc_md3701}{}\doxysubsection{Shell Recorder}\label{doc_TESTING_md_autotoc_md3701}
The more elegant way to specify script tests are via the so-\/called Shell Recorder using Markdown Syntax.

See here.\hypertarget{doc_TESTING_md_autotoc_md3702}{}\doxysubsection{Fuzz Testing}\label{doc_TESTING_md_autotoc_md3702}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md3703}{}\doxysubsection{ASAN}\label{doc_TESTING_md_autotoc_md3703}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md3704}{}\doxysubsubsection{mac\+OS}\label{doc_TESTING_md_autotoc_md3704}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md3705}{}\doxysubsection{CBMC}\label{doc_TESTING_md_autotoc_md3705}
For bounded model checking tests, see {\ttfamily scripts/cbmc}.\hypertarget{doc_TESTING_md_autotoc_md3706}{}\doxysubsection{Static Code Checkers}\label{doc_TESTING_md_autotoc_md3706}
There is a number of static code checkers available for all kind of programming languages. The following section show how the most common ones can be used with {\ttfamily libelektra}.\hypertarget{doc_TESTING_md_autotoc_md3707}{}\doxysubsubsection{Cppcheck}\label{doc_TESTING_md_autotoc_md3707}
...
Now you can view the html report by opening {\ttfamily index.\+html} in the specified folder to get an overview of the issues found in the whole project.\hypertarget{doc_TESTING_md_autotoc_md3708}{}\doxysubsubsection{OCLint}\label{doc_TESTING_md_autotoc_md3708}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md3709}{}\doxysubsubsection{scan-\/build}\label{doc_TESTING_md_autotoc_md3709}
...
Alternatively, you can also open the {\ttfamily index.\+html} file in the aforementioned folder, but using the tool the report is enriched with further information.\hypertarget{doc_TESTING_md_autotoc_md3710}{}\doxysubsubsection{Sonar\+Lint}\label{doc_TESTING_md_autotoc_md3710}
\href{http://www.sonarlint.org/}{\texttt{ Sonar\+Lint}} is a static code checker primarily intended for Java. It is usually used by installing the corresponding plugin for the used IDE, then there is no further configuration required.\hypertarget{doc_TESTING_md_autotoc_md3711}{}\doxysubsection{Randoop}\label{doc_TESTING_md_autotoc_md3711}
For using the unit test generator randoop with the jna bindings, see {\ttfamily scripts/randoop/randoop}.\hypertarget{doc_TESTING_md_autotoc_md3712}{}\doxysubsection{Code Coverage}\label{doc_TESTING_md_autotoc_md3712}
Run\+:
...
The HTML files can be found in the build directory in the folder {\ttfamily coverage}.\hypertarget{doc_TESTING_md_autotoc_md3713}{}\doxysection{See Also}\label{doc_TESTING_md_autotoc_md3713}

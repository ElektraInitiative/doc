...
The best way of integrating Elektra into applications is to \mbox{\hyperlink{doc_help_elektra-glossary_md}{elektrify}} them.

To use Elektra with non-\/elektrified applications we can also let Elektra sync configuration files on the file system and Elektra\textquotesingle{}s key-\/value storage. Applications can then read and write the configuration files and changes in the key database will be picked up by applications and vice-\/versa.

The heart of the approach is the so-\/called {\itshape mounting} of configuration files into the key database.

Let us start with a motivating example first\+:\hypertarget{doc_tutorials_mount_md_autotoc_md4376}{}\doxysection{Mount the Lookup Table for Hostnames}\label{doc_tutorials_mount_md_autotoc_md4376}
...
Because of that only root can mount files.\hypertarget{doc_tutorials_mount_md_autotoc_md4377}{}\doxysection{Resolver}\label{doc_tutorials_mount_md_autotoc_md4377}
...
But resolvers are not the only plugins Elektra uses\+:\hypertarget{doc_tutorials_mount_md_autotoc_md4378}{}\doxysection{Plugins}\label{doc_tutorials_mount_md_autotoc_md4378}
...
When you mount a file you can tell Elektra which plugins it should use for reading and writing to configuration files.\hypertarget{doc_tutorials_mount_md_autotoc_md4379}{}\doxysubsubsection{Metadata}\label{doc_tutorials_mount_md_autotoc_md4379}
...
If you want to find out more about validation I recommend reading \mbox{\hyperlink{doc_tutorials_validation_md}{this}} tutorial next.\hypertarget{doc_tutorials_mount_md_autotoc_md4380}{}\doxysubsubsection{Backends}\label{doc_tutorials_mount_md_autotoc_md4380}
The plugins together with the configuration file form a {\itshape backend}. The backend determines how Elektra stores data below a mount point. You can examine every mount points backend by looking at the configuration below {\ttfamily system\+:/elektra/mountpoints/$<$mount point$>$/}.\hypertarget{doc_tutorials_mount_md_autotoc_md4381}{}\doxysection{Limitations}\label{doc_tutorials_mount_md_autotoc_md4381}

\label{doc_decisions_1_problem_clear_change_tracking_md_md_doc_decisions_1_problem_clear_change_tracking}%
\Hypertarget{doc_decisions_1_problem_clear_change_tracking_md_md_doc_decisions_1_problem_clear_change_tracking}%
 \hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1908}{}\doxysection{Problem}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1908}
...
\item {\ttfamily spec\+:/}
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1909}{}\doxysection{Constraints}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1909}
...
The library {\ttfamily libelektra-\/core} must be kept minimal.\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1910}{}\doxysection{Assumptions}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1910}
...
\item False positivies are okay
\begin{DoxyItemize}
\item this may happend when some keys have been changed by the user, but have subsequentially been \char`\"{}unchanged\char`\"{} by a transformation plugin Scenario\+: plugin that converts {\ttfamily false}$<$-\/$>${\ttfamily 0} and {\ttfamily true}$<$-\/$>${\ttfamily 1}
\begin{DoxyItemize}
...
\item We assume consumers of the changetracking API have access to both the previous and the new value of a changed key. Therefore, they could detect these false positive cases, if the need to.
\end{DoxyItemize}
\end{DoxyItemize}
\item False negatives (missed changes) are not okay
\end{DoxyItemize}\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1911}{}\doxysection{Solutions -\/ Storage}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1911}
\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1912}{}\doxysubsection{Utilize already existing $<$tt$>$backend\+Data-\/$>$keys$<$/tt$>$}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1912}
...
A problem with this approach is that the internally stored keys are recreated as new instances every time {\ttfamily kdb\+Get} is called.\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1913}{}\doxysubsection{Combine with internal cache}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1913}
We already decided that we want to have an internal deep-\/duped keysets of all the keys we returned. See \mbox{\hyperlink{doc_decisions_4_decided_internal_cache_md}{internal cache decision}}.

The difference to {\ttfamily backend\+Data-\/$>$keys} is that this cache is not recreated each time {\ttfamily kdb\+Get} is called.\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1914}{}\doxysubsection{Have a seperate storage for changetracking}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1914}
Have a global keyset with deep-\/duped keys that is purely used for changetracking and nothing else.\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1915}{}\doxysubsection{Store changes as meta keys}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1915}
When something changes for the first time, store the original value as a metakey to every key. Not yet clear how we handle changes to metadata then. Also not really possible for keysets.\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1916}{}\doxysubsection{Implement changetracking as a mechanismn on the $<$tt$>$\+Key$<$/tt$>$ and $<$tt$>$\+Key\+Set$<$/tt$>$ datastructures}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1916}
...
\end{DoxyItemize}\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1917}{}\doxysection{Solutions -\/ Implementation}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1917}
\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1918}{}\doxysubsection{Implement directly within $<$tt$>$libelektra-\/kdb$<$/tt$>$}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1918}
Implement all the logic for changetracking directly within {\ttfamily libelektra-\/kdb}.\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1919}{}\doxysubsection{Implement as a seperate plugin}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1919}
...
As every hook plugin can define its own contract, in theory all storage forms mentioned in the previous chapter should be possible to implement. We could just point the plugin to {\ttfamily backends\+Data-\/$>$keys} or the internal cache if we go down that route.\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1920}{}\doxysection{Solutions -\/ Query}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1920}
\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1921}{}\doxysubsection{Provide an API within $<$tt$>$libelektra-\/kdb$<$/tt$>$}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1921}
The API should be useable both by plugins and applications utilizing ELektra. It does not matter whether the changetracking is implemented as part of {\ttfamily libelektra-\/kdb} or as a seperate plugin. The API may look something like this\+:
...
\end{DoxyCode}
\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1922}{}\doxysubsection{Provide query methods as part of a seperat plugin}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1922}
...
\end{DoxyItemize}\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1923}{}\doxysection{Decision}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1923}
\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1924}{}\doxysection{Rationale}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1924}
\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1925}{}\doxysection{Implications}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1925}

\begin{DoxyItemize}
\item 
\item 
\item 
\end{DoxyItemize}\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1926}{}\doxysection{Related Decisions}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1926}
...
\end{DoxyItemize}\hypertarget{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1927}{}\doxysection{Notes}\label{doc_decisions_1_problem_clear_change_tracking_md_autotoc_md1927}

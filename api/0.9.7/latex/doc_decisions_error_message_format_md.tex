...
\Hypertarget{doc_decisions_error_message_format_md_md_doc_decisions_error_message_format}%
 \hypertarget{doc_decisions_error_message_format_md_autotoc_md1821}{}\doxysection{Problem}\label{doc_decisions_error_message_format_md_autotoc_md1821}
Too verbose error message Currently for every error, 9 lines are shown in which most of them are not relevant to end users/administrators. One goal is to reduce the verbosity of such messages and let users/administrators see only information they need.\hypertarget{doc_decisions_error_message_format_md_autotoc_md1822}{}\doxysection{Constraints}\label{doc_decisions_error_message_format_md_autotoc_md1822}

\begin{DoxyItemize}
\item Supporting multiple programming languages
\item Plugin System
\item Error Code should be preserved
\end{DoxyItemize}\hypertarget{doc_decisions_error_message_format_md_autotoc_md1823}{}\doxysection{Assumptions}\label{doc_decisions_error_message_format_md_autotoc_md1823}
\hypertarget{doc_decisions_error_message_format_md_autotoc_md1824}{}\doxysection{Considered Alternatives}\label{doc_decisions_error_message_format_md_autotoc_md1824}
...
\end{DoxyItemize}\hypertarget{doc_decisions_error_message_format_md_autotoc_md1825}{}\doxysection{Decision}\label{doc_decisions_error_message_format_md_autotoc_md1825}
...
To avoid losing information, the user can use the command line argument {\ttfamily -\/v} (verbose) to show {\ttfamily Mountpoint}, {\ttfamily Configfile} in addition to the current error message. Furthermore a developer can use the command line argument {\ttfamily -\/d} (debug) to show {\ttfamily At} for debugging purposes.\hypertarget{doc_decisions_error_message_format_md_autotoc_md1826}{}\doxysection{Rationale}\label{doc_decisions_error_message_format_md_autotoc_md1826}
The new error message is much more succinct which gives end users more relevant information. Furthermore the solution approach still holds all necessary information if requested by users.\hypertarget{doc_decisions_error_message_format_md_autotoc_md1827}{}\doxysection{Implications}\label{doc_decisions_error_message_format_md_autotoc_md1827}
{\ttfamily Description} will be incorporated into {\ttfamily Reason} whereas the {\ttfamily Module} will be incorporated into the general sentence starting the error message.\hypertarget{doc_decisions_error_message_format_md_autotoc_md1828}{}\doxysection{Related Decisions}\label{doc_decisions_error_message_format_md_autotoc_md1828}
...
\end{DoxyItemize}\hypertarget{doc_decisions_error_message_format_md_autotoc_md1829}{}\doxysection{Notes}\label{doc_decisions_error_message_format_md_autotoc_md1829}

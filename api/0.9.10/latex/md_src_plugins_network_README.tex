...
Furthermore {\ttfamily getaddrinfo()} is used in {\ttfamily check/port} to resolve a port by its service name which is defined under {\ttfamily /etc/services}. The portname is translated to the respective portnumber. The plugin can be used to check for valid port numbers and if the set port is free to use.\hypertarget{md_src_plugins_network_README_autotoc_md371}{}\doxysection{Purpose}\label{md_src_plugins_network_README_autotoc_md371}
...
Many network address translators coexist. In {\ttfamily POSIX.\+1-\/2001} a powerful address translator is provided with the interface {\ttfamily getaddrinfo()}. It is a common network address translation for both IPv4 and IPv6. We used it to implement this plugin.\hypertarget{md_src_plugins_network_README_autotoc_md372}{}\doxysection{Usage}\label{md_src_plugins_network_README_autotoc_md372}
Every key tagged with the metakey {\ttfamily check/ipaddr} will be checked using {\ttfamily getaddrinfo()}. If additionally the values {\ttfamily ipv4} or {\ttfamily ipv6} are supplied, the address family will be specified.\hypertarget{md_src_plugins_network_README_autotoc_md373}{}\doxysubsection{Example}\label{md_src_plugins_network_README_autotoc_md373}
...
If {\ttfamily check/port/listen} is specified, the plugin will check if the application can be started and listen on the given port.\hypertarget{md_src_plugins_network_README_autotoc_md374}{}\doxysection{Future Work}\label{md_src_plugins_network_README_autotoc_md374}

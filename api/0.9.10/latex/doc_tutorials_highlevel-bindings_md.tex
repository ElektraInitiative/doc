...
Writing bindings for high-\/level API is different from writing bindings for other parts of Elektra. This is mainly because the high-\/level API has different goals.\hypertarget{doc_tutorials_highlevel-bindings_md_autotoc_md3814}{}\doxysection{Goals}\label{doc_tutorials_highlevel-bindings_md_autotoc_md3814}
...
\end{DoxyItemize}\hypertarget{doc_tutorials_highlevel-bindings_md_autotoc_md3815}{}\doxysection{When to write Bindings}\label{doc_tutorials_highlevel-bindings_md_autotoc_md3815}
...
If not, designing an API to meet our goals is up to you. While designing the API you should keep in mind that you can always use code-\/generator ({\ttfamily kdb gen}) templates like the C API does.\hypertarget{doc_tutorials_highlevel-bindings_md_autotoc_md3816}{}\doxysection{How to write Bindings}\label{doc_tutorials_highlevel-bindings_md_autotoc_md3816}
Since the C high-\/level API consists of a shared library API and a code-\/generated part, your binding will also have these two parts.\hypertarget{doc_tutorials_highlevel-bindings_md_autotoc_md3817}{}\doxysubsection{Bindings for the $<$tt$>$highlevel$<$/tt$>$ Library}\label{doc_tutorials_highlevel-bindings_md_autotoc_md3817}
...
The generated code should still be understandable. So if writing a more idiomatic API on top of the direct mapping, significantly simplifies the generated code (and maybe also the template), you should write such an API.\hypertarget{doc_tutorials_highlevel-bindings_md_autotoc_md3818}{}\doxysubsection{Bindings for the $<$tt$>$lowlevel$<$/tt$>$ Library}\label{doc_tutorials_highlevel-bindings_md_autotoc_md3818}
...
Remember that Elektra has internal iterators (for metadata+keysets) but in general we prefer external iterators by either copying the Key\+Set per iterator or using {\ttfamily ks\+At\+Cursor}.\hypertarget{doc_tutorials_highlevel-bindings_md_autotoc_md3819}{}\doxysubsection{Variadic Functions}\label{doc_tutorials_highlevel-bindings_md_autotoc_md3819}
...
\end{DoxyEnumerate}\hypertarget{doc_tutorials_highlevel-bindings_md_autotoc_md3820}{}\doxysubsection{Creating the Code-\/\+Generator Template}\label{doc_tutorials_highlevel-bindings_md_autotoc_md3820}

...
\Hypertarget{doc_TESTING_md_md_doc_TESTING}%
 \hypertarget{doc_TESTING_md_autotoc_md4165}{}\doxysection{Introduction}\label{doc_TESTING_md_autotoc_md4165}
Libraries need pervasive testing for continuous improvement. Any problem found and behavior described must be written down as test so that it is assured that no new regressions will be added.\hypertarget{doc_TESTING_md_autotoc_md4166}{}\doxysection{Running Tests}\label{doc_TESTING_md_autotoc_md4166}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4167}{}\doxysection{Required Environment}\label{doc_TESTING_md_autotoc_md4167}
...
\end{DoxyEnumerate}\hypertarget{doc_TESTING_md_autotoc_md4168}{}\doxysection{Manual Testing}\label{doc_TESTING_md_autotoc_md4168}
...
Pay attention that sourcing depends on the operating system or rather the shell. For example on standard Free\+BSD 11.\+3 you have to execute {\ttfamily sh} in the root of the repository first. Then do {\itshape not} use the {\ttfamily source} command but the point {\ttfamily .} as explained above.\hypertarget{doc_TESTING_md_autotoc_md4169}{}\doxysection{Using debuggers}\label{doc_TESTING_md_autotoc_md4169}
...
You can also use Docker to set up such an environment. There is \mbox{\hyperlink{doc_tutorials_run_all_tests_with_docker_md}{a tutorial}} that guides you through the necessary steps.\hypertarget{doc_TESTING_md_autotoc_md4171}{}\doxysection{Adding Tests}\label{doc_TESTING_md_autotoc_md4171}
...
By using {\ttfamily TEST\+\_\+\+README} in {\ttfamily add\+\_\+plugin} (also enabled by default), \href{https://master.libelektra.org/tests/shell/shell_recorder/tutorial_wrapper}{\texttt{ Markdown Shell Recorder}} are expected to be in the README.\+md of the plugin.\hypertarget{doc_TESTING_md_autotoc_md4172}{}\doxysection{Conventions}\label{doc_TESTING_md_autotoc_md4172}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4173}{}\doxysection{Strategy}\label{doc_TESTING_md_autotoc_md4173}
The testing must happen on every level of the software to achieve a maximum coverage with the available time. In the rest of the document we describe the different levels and where these tests are.\hypertarget{doc_TESTING_md_autotoc_md4174}{}\doxysubsection{CFramework}\label{doc_TESTING_md_autotoc_md4174}
This is basically a bunch of assertion macros and some output facilities. It is written in pure C and very lightweight.

It is located here.\hypertarget{doc_TESTING_md_autotoc_md4175}{}\doxysubsection{ABI Tests}\label{doc_TESTING_md_autotoc_md4175}
...
The tests are located here.\hypertarget{doc_TESTING_md_autotoc_md4176}{}\doxysubsection{C Unit Tests}\label{doc_TESTING_md_autotoc_md4176}
...
They are located here.\hypertarget{doc_TESTING_md_autotoc_md4177}{}\doxysubsubsection{Internal Functions}\label{doc_TESTING_md_autotoc_md4177}
...
get exported. Functions not starting with this prefix are internal only and therefore not visible in the test cases. Test internal functionality by including the corresponding C file.\hypertarget{doc_TESTING_md_autotoc_md4178}{}\doxysubsection{Module Tests}\label{doc_TESTING_md_autotoc_md4178}
...
Use the CMake macro {\ttfamily add\+\_\+plugintest} for adding these tests.\hypertarget{doc_TESTING_md_autotoc_md4179}{}\doxysubsection{C++ Unit Tests}\label{doc_TESTING_md_autotoc_md4179}
...
Use the CMake macro {\ttfamily add\+\_\+gtest} for adding these tests.\hypertarget{doc_TESTING_md_autotoc_md4180}{}\doxysubsection{Script Tests}\label{doc_TESTING_md_autotoc_md4180}
...
See here.\hypertarget{doc_TESTING_md_autotoc_md4181}{}\doxysubsection{Shell Recorder}\label{doc_TESTING_md_autotoc_md4181}
The more elegant way to specify script tests are via the so-\/called Shell Recorder using Markdown Syntax.

See here.\hypertarget{doc_TESTING_md_autotoc_md4182}{}\doxysubsection{Fuzz Testing}\label{doc_TESTING_md_autotoc_md4182}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4183}{}\doxysubsection{ASAN}\label{doc_TESTING_md_autotoc_md4183}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4184}{}\doxysubsubsection{mac\+OS}\label{doc_TESTING_md_autotoc_md4184}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4185}{}\doxysubsection{CBMC}\label{doc_TESTING_md_autotoc_md4185}
For bounded model checking tests, see {\ttfamily scripts/cbmc}.\hypertarget{doc_TESTING_md_autotoc_md4186}{}\doxysubsection{Static Code Checkers}\label{doc_TESTING_md_autotoc_md4186}
There is a number of static code checkers available for all kind of programming languages. The following section show how the most common ones can be used with {\ttfamily libelektra}.\hypertarget{doc_TESTING_md_autotoc_md4187}{}\doxysubsubsection{Cppcheck}\label{doc_TESTING_md_autotoc_md4187}
...
Now you can view the html report by opening {\ttfamily index.\+html} in the specified folder to get an overview of the issues found in the whole project.\hypertarget{doc_TESTING_md_autotoc_md4188}{}\doxysubsubsection{OCLint}\label{doc_TESTING_md_autotoc_md4188}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4189}{}\doxysubsubsection{scan-\/build}\label{doc_TESTING_md_autotoc_md4189}
...
Alternatively, you can also open the {\ttfamily index.\+html} file in the aforementioned folder, but using the tool the report is enriched with further information.\hypertarget{doc_TESTING_md_autotoc_md4190}{}\doxysubsubsection{Sonar\+Lint}\label{doc_TESTING_md_autotoc_md4190}
\href{https://www.sonarlint.org/}{\texttt{ Sonar\+Lint}} is a static code checker primarily intended for Java. It is usually used by installing the corresponding plugin for the used IDE, then there is no further configuration required.\hypertarget{doc_TESTING_md_autotoc_md4191}{}\doxysubsection{Randoop}\label{doc_TESTING_md_autotoc_md4191}
For using the unit test generator randoop with the jna bindings, see {\ttfamily scripts/randoop/randoop}.\hypertarget{doc_TESTING_md_autotoc_md4192}{}\doxysubsection{Code Coverage}\label{doc_TESTING_md_autotoc_md4192}
Run\+:
...
The HTML files can be found in the build directory in the folder {\ttfamily coverage}.\hypertarget{doc_TESTING_md_autotoc_md4193}{}\doxysection{See Also}\label{doc_TESTING_md_autotoc_md4193}

...
This is because {\ttfamily ld} tries to link {\ttfamily /usr/lib/libaugeas.so.\+0} which is an older version of Augeas. Simply add the path to the newer library to your ld search paths (consult your system documentation on how to do this).\hypertarget{md_src_plugins_augeas_README_autotoc_md47}{}\doxysection{Mounting and Configuration}\label{md_src_plugins_augeas_README_autotoc_md47}
In order to mount the hosts file with the augeas plugin, issue the following command\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{kdb mount /etc/hosts system/hosts augeas lens=Hosts.lns}
\end{DoxyCode}
...
\DoxyCodeLine{\#> Validation Syntactic: Lens not found}
\end{DoxyCode}


This happens because the plugin does not know which lens it should use to read and write the configuration. For that reason, the lens configuration option was supplied together with the mount command.\hypertarget{md_src_plugins_augeas_README_autotoc_md48}{}\doxysection{Restrictions}\label{md_src_plugins_augeas_README_autotoc_md48}
\hypertarget{md_src_plugins_augeas_README_autotoc_md49}{}\doxysubsection{Inner Node Values}\label{md_src_plugins_augeas_README_autotoc_md49}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{kdb set system/hosts/1 somevalue}
\DoxyCodeLine{\#> The command set terminated unsuccessfully with the info: Error (\#85) occurred!}
\DoxyCodeLine{\#> Description: an Augeas error occurred}
\DoxyCodeLine{\#> Module: storage}
\DoxyCodeLine{\#> At: /path/augeas.c:166}
\DoxyCodeLine{\#> an Augeas error occurred: Malformed child node '1'}
\end{DoxyCode}


The operation simply fails with an undescriptive error.\hypertarget{md_src_plugins_augeas_README_autotoc_md50}{}\doxysubsection{Leaky Abstraction of Order}\label{md_src_plugins_augeas_README_autotoc_md50}
...
\DoxyCodeLine{kdbSet system/hosts}
\end{DoxyCode}
\hypertarget{md_src_plugins_augeas_README_autotoc_md51}{}\doxysection{Planned Improvements}\label{md_src_plugins_augeas_README_autotoc_md51}

...
\hline
{\em 0} & if there was no update -\/ no changes are made to the Key\+Set then \\
\hline
...
With {\ttfamily parent\+Key} you can specify which part of the given keyset is of interest for you. Then you promise to only modify or remove keys below this key. All others would be passed back as they were retrieved by \mbox{\hyperlink{group__kdb_ga28e385fd9cb7ccfe0b2f1ed2f62453a1}{kdb\+Get()}}. Cascading keys (starting with /) will set the path in all namespaces. {\ttfamily /} will commit all keys. Meta-\/names in {\ttfamily parent\+Key} will be rejected (error C01320). Empty/\+Invalid Keys will also be rejected (error C01320).

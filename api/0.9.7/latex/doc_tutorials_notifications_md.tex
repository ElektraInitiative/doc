...
\Hypertarget{doc_tutorials_notifications_md_md_doc_tutorials_notifications}%
 \hypertarget{doc_tutorials_notifications_md_autotoc_md3666}{}\doxysection{Preface}\label{doc_tutorials_notifications_md_autotoc_md3666}
{\bfseries{The features described in this document are experimental.}}

This document explains how notifications are implemented in Elektra and how they can be used by application developers.\hypertarget{doc_tutorials_notifications_md_autotoc_md3667}{}\doxysection{Notifications -\/ Overview \& Concept}\label{doc_tutorials_notifications_md_autotoc_md3667}
...
Make sure to compile/link with {\ttfamily pkg-\/config -\/-\/libs -\/-\/cflags elektra-\/io-\/uv}.\hypertarget{doc_tutorials_notifications_md_autotoc_md3670}{}\doxysection{How to receive notifications}\label{doc_tutorials_notifications_md_autotoc_md3670}
...
In order to handle change notifications a developer can either register a variable or a callback.\hypertarget{doc_tutorials_notifications_md_autotoc_md3671}{}\doxysubsection{Register a variable}\label{doc_tutorials_notifications_md_autotoc_md3671}
...
\end{DoxyCode}
\hypertarget{doc_tutorials_notifications_md_autotoc_md3673}{}\doxysubsection{How-\/\+To\+: Reload KDB when Elektra\textquotesingle{}s configuration has changed}\label{doc_tutorials_notifications_md_autotoc_md3673}
...
Building on these findings we will now present guidelines for preventing emergent misbehavior when using the notification API and callbacks in particular.\hypertarget{doc_tutorials_notifications_md_autotoc_md3675}{}\doxysubsection{Guideline 1\+: Avoid callbacks}\label{doc_tutorials_notifications_md_autotoc_md3675}
\begin{quote}
...
Generally waiting before reacting to changes reduces the risk for unwanted synchronization by decoupling the application temporally. Waiting can be implemented using random time delays which further promotes decoupling since applications react at different points in time to changes. Waiting can also be implemented using a flag\+: Callbacks set the flag and when the control flow is in the main loop again, the pending updates are applied and the flag is cleared.\hypertarget{doc_tutorials_notifications_md_autotoc_md3677}{}\doxysubsection{Guideline 3\+: Avoid updates as reaction to change}\label{doc_tutorials_notifications_md_autotoc_md3677}
\begin{quote}
...
This guideline applies especially to callbacks but is also relevant when variables are polled for changes by the application.\hypertarget{doc_tutorials_notifications_md_autotoc_md3678}{}\doxysubsection{Guideline 4\+: Do not use notifications for synchronization}\label{doc_tutorials_notifications_md_autotoc_md3678}
\begin{quote}
...
For example, this happens when applications synchronize themselves at startup by incrementing a counter in the key database. When a certain limit of application instances is reached the applications proceed with different behavior. If this behavior affects other applications phase change has occurred.\hypertarget{doc_tutorials_notifications_md_autotoc_md3679}{}\doxysubsection{Guideline 5\+: Apply changes immediately}\label{doc_tutorials_notifications_md_autotoc_md3679}
\begin{quote}
...
When a configuration setting is updated within an application this guideline suggests to write the change immediately to the key database using {\ttfamily \mbox{\hyperlink{group__kdb_ga11436b058408f83d303ca5e996832bcf}{kdb\+Set()}}}. This ensures that other applications have the same view of the key database and operate on current settings.\hypertarget{doc_tutorials_notifications_md_autotoc_md3680}{}\doxysubsection{Guideline 6\+: Be careful on what to call inside callbacks}\label{doc_tutorials_notifications_md_autotoc_md3680}
\begin{quote}
...
Closing and cleaning up the KDB handle will cause an application crash because the control flow returns from the callback to now removed code. While this can be considered an implementation detail it aligns with \href{\#guidline-2-wait-before-reacting-to-changes}{\texttt{ Guideline 2}} since reinitialization of KDB uses more resources than other operations like {\ttfamily \mbox{\hyperlink{group__kdb_ga28e385fd9cb7ccfe0b2f1ed2f62453a1}{kdb\+Get()}}} or {\ttfamily \mbox{\hyperlink{group__kdb_ga11436b058408f83d303ca5e996832bcf}{kdb\+Set()}}}.\hypertarget{doc_tutorials_notifications_md_autotoc_md3681}{}\doxysection{Logging}\label{doc_tutorials_notifications_md_autotoc_md3681}
...
Now both reading from and writing to Elektra\textquotesingle{}s key database is logged.\hypertarget{doc_tutorials_notifications_md_autotoc_md3682}{}\doxysection{How to create your own I/\+O Binding}\label{doc_tutorials_notifications_md_autotoc_md3682}

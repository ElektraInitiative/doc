...
\Hypertarget{doc_dev_data-structures_md_md_doc_dev_data_structures}%
 For an introduction, please \mbox{\hyperlink{doc_dev_classes_md}{read first about elektra classes}}. You might want to read \mbox{\hyperlink{doc_dev_architecture_md}{about architecture first}}.\hypertarget{doc_dev_data-structures_md_autotoc_md2585}{}\doxysection{Introduction}\label{doc_dev_data-structures_md_autotoc_md2585}
Data structures define the common layer in Elektra. They are used for transferring configuration between Elektra and applications, but also between plugins.\hypertarget{doc_dev_data-structures_md_autotoc_md2586}{}\doxysubsection{ADT}\label{doc_dev_data-structures_md_autotoc_md2586}
...
{\ttfamily Key\+Set} combines the best of both worlds\+: {\ttfamily Key\+Set} is implemented as a sorted array and uses an \href{\#order-preserving-minimal-perfect-hash-map-aka-opmphm}{\texttt{ order-\/preserving minimal perfect hash map (OPMPHM)}} for lookups.\hypertarget{doc_dev_data-structures_md_autotoc_md2587}{}\doxysubsection{ABI compatibility}\label{doc_dev_data-structures_md_autotoc_md2587}
...
\end{DoxyItemize}\hypertarget{doc_dev_data-structures_md_autotoc_md2592}{}\doxysubsection{Consistency}\label{doc_dev_data-structures_md_autotoc_md2592}
...
\end{DoxyCode}
\hypertarget{doc_dev_data-structures_md_autotoc_md2594}{}\doxysection{Order Preserving Minimal Perfect Hash Map (aka OPMPHM)}\label{doc_dev_data-structures_md_autotoc_md2594}
...
{\ttfamily opmphm\+OptR (n)} and {\ttfamily opmphm\+OptC (n)} are heuristic functions constructed through benchmarks. Optimal is only one mapping step invocation in 99.\+5\% of the observed cases. The benchmarks took arbitrary uniform distributed initial seeds and the heuristic functions are made to work with almost every seed.\hypertarget{doc_dev_data-structures_md_autotoc_md2595}{}\doxysubsection{The Build}\label{doc_dev_data-structures_md_autotoc_md2595}
\hypertarget{doc_dev_data-structures_md_autotoc_md2596}{}\doxysubsubsection{Initialization}\label{doc_dev_data-structures_md_autotoc_md2596}
...
To initialize the OPMPHM build the {\ttfamily Opmphm\+Init} must be set with information about your data. Set your data array {\ttfamily Opmphm\+Init-\/$>$data} and the element name extraction function {\ttfamily Opmphm\+Init-\/$>$get\+Name}, which should extract the string from a single data array entry. Provide a good seed in {\ttfamily Opmphm\+Init-\/$>$init\+Seed}, needed in the next step.\hypertarget{doc_dev_data-structures_md_autotoc_md2597}{}\doxysubsubsection{Mapping}\label{doc_dev_data-structures_md_autotoc_md2597}
The function {\ttfamily opmphm\+Mapping} uses your seed (the {\ttfamily Opmphm\+Init-\/$>$seed} will be changed) and tries to construct the random acyclic r-\/uniform r-\/partite hypergraph, this might not succeed, on cycles just call it again.\hypertarget{doc_dev_data-structures_md_autotoc_md2598}{}\doxysubsubsection{Assignment}\label{doc_dev_data-structures_md_autotoc_md2598}
...
After the build the Opmphm\+Init and Opmphm\+Graph should be freed. The OPMPHM is now ready for constant lookups with the {\ttfamily opmphm\+Lookup ()}.\hypertarget{doc_dev_data-structures_md_autotoc_md2599}{}\doxysubsection{The Rebuild}\label{doc_dev_data-structures_md_autotoc_md2599}

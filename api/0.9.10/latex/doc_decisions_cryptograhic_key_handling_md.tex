...
\Hypertarget{doc_decisions_cryptograhic_key_handling_md_md_doc_decisions_cryptograhic_key_handling}%
 \hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1736}{}\doxysection{Problem}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1736}
The crypto plugin applies cryptographic operations to Keys and Key\+Sets. In order to do that it needs keys and initialization vectors (IV). The problem is how to retrieve or derivate those keys in a safe way and how to pass them on to the underlying crypto libraries (Open\+SSL and libgcrypt at the time of writing).\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1737}{}\doxysection{Constraints}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1737}
...
\end{DoxyItemize}\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1738}{}\doxysection{Assumptions}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1738}
...
\end{DoxyItemize}\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1739}{}\doxysection{Considered Alternatives}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1739}
...
Also we considered using the gpg-\/agent (or pcscd, which uses a similar protocol) for providing asymmetric cryptographic operations. The problem with gpg-\/agent is that it only provides operations which require the private part of the key-\/pair (i.\+e. signing and decrypting). We would still have to implement the counterpart operations (i.\+e. verifying and encrypting) on our own. Starting the gpg-\/agent and the whole interprocess communication is either tedious work (when implemented by oneself) or adds another dependency (when using libassuan). So we do not consider the gpg-\/agent to be a viable option.\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1740}{}\doxysection{Decision}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1740}
\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1741}{}\doxysubsection{General Approach}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1741}
...
The PBKDF2 needs an iteration number and a salt in order to work. Those values will be stored per Key as Meta\+Key.\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1742}{}\doxysubsection{Implementation Details}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1742}
...
The constraints considering the key handling are met.\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1744}{}\doxysection{Implications}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1744}
To manipulate the plugin configuration during the {\bfseries{mount phase}} a hook is required within the {\ttfamily Backend\+Builder}. See pull request \href{https://github.com/ElektraInitiative/libelektra/pull/733}{\texttt{ \#733}} for full discussion.\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1745}{}\doxysection{Related Decisions}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1745}
\hypertarget{doc_decisions_cryptograhic_key_handling_md_autotoc_md1746}{}\doxysection{Notes}\label{doc_decisions_cryptograhic_key_handling_md_autotoc_md1746}

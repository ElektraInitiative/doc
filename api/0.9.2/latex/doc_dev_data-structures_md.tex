For an introduction, please \mbox{\hyperlink{doc_dev_classes_md}{read first about elektra classes}}. You might want to read \mbox{\hyperlink{doc_dev_architecture_md}{about architecture first}}.\hypertarget{doc_dev_data-structures_md_autotoc_md1434}{}\doxysection{Introduction}\label{doc_dev_data-structures_md_autotoc_md1434}
Data structures define the common layer in Elektra. They are used to transfer configuration between Elektra and applications, but also between plugins.\hypertarget{doc_dev_data-structures_md_autotoc_md1435}{}\doxysubsection{A\+DT}\label{doc_dev_data-structures_md_autotoc_md1435}
...
Currently the {\ttfamily Key\+Set} is implemented as a sorted array. It is fast on appending and iterating, and has nearly no size-\/overhead. To improve the lookup-\/time, an additional {\bfseries{hash}} will be used.\hypertarget{doc_dev_data-structures_md_autotoc_md1436}{}\doxysubsection{A\+B\+I compatibility}\label{doc_dev_data-structures_md_autotoc_md1436}
...
\end{DoxyItemize}\hypertarget{doc_dev_data-structures_md_autotoc_md1440}{}\doxysubsection{Consistency}\label{doc_dev_data-structures_md_autotoc_md1440}
...
Continue reading \mbox{\hyperlink{doc_dev_error-handling_md}{with the error handling}}.\hypertarget{doc_dev_data-structures_md_autotoc_md1446}{}\doxysection{Order Preserving Minimal Perfect Hash Map (aka O\+P\+M\+P\+H\+M)}\label{doc_dev_data-structures_md_autotoc_md1446}
...
{\ttfamily opmphm\+OptR (n)} and {\ttfamily opmphm\+OptC (n)} are heuristic functions constructed through benchmarks. Optimal is only one mapping step invocation in 99.\+5\% of the observed cases. The benchmarks took arbitrary uniform distributed initial seeds and the heuristic functions are made to work with almost every seed.\hypertarget{doc_dev_data-structures_md_autotoc_md1447}{}\doxysubsection{The Build}\label{doc_dev_data-structures_md_autotoc_md1447}
\hypertarget{doc_dev_data-structures_md_autotoc_md1448}{}\doxysubsubsection{Initialization}\label{doc_dev_data-structures_md_autotoc_md1448}
...
To initialize the O\+P\+M\+P\+HM build the {\ttfamily Opmphm\+Init} must be set with information about your data. Set your data array {\ttfamily Opmphm\+Init-\/$>$data} and the element name extraction function {\ttfamily Opmphm\+Init-\/$>$get\+Name}, which should extract the string from a single data array entry. Provide a good seed in {\ttfamily Opmphm\+Init-\/$>$init\+Seed}, needed in the next step.\hypertarget{doc_dev_data-structures_md_autotoc_md1449}{}\doxysubsubsection{Mapping}\label{doc_dev_data-structures_md_autotoc_md1449}
The function {\ttfamily opmphm\+Mapping} uses your seed (the {\ttfamily Opmphm\+Init-\/$>$seed} will be changed) and tries to construct the random acyclic r-\/uniform r-\/partite hypergraph, this might not succeed, on cycles just call it again.\hypertarget{doc_dev_data-structures_md_autotoc_md1450}{}\doxysubsubsection{Assignment}\label{doc_dev_data-structures_md_autotoc_md1450}
...
After the build the Opmphm\+Init and Opmphm\+Graph should be freed. The O\+P\+M\+P\+HM is now ready for constant lookups with the {\ttfamily opmphm\+Lookup ()}.\hypertarget{doc_dev_data-structures_md_autotoc_md1451}{}\doxysubsection{The Rebuild}\label{doc_dev_data-structures_md_autotoc_md1451}

...
\hypertarget{md_src_plugins_base64_README_autotoc_md58}{}\doxysubsection{Introduction}\label{md_src_plugins_base64_README_autotoc_md58}
...
The {\ttfamily base64} plugin encodes binary values before {\ttfamily kdb set} writes the configuration to the file. The values are decoded back to their original value after {\ttfamily kdb get} has read from the configuration file.\hypertarget{md_src_plugins_base64_README_autotoc_md59}{}\doxysubsection{Usage}\label{md_src_plugins_base64_README_autotoc_md59}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_base64_README_autotoc_md60}{}\doxysubsection{Modes}\label{md_src_plugins_base64_README_autotoc_md60}
...
. By default the plugin uses escaping mode which has the advantage that a storage plugin does not have to change its behavior at all to work in conjunction with Base64.\hypertarget{md_src_plugins_base64_README_autotoc_md61}{}\doxysubsubsection{Escaping Mode}\label{md_src_plugins_base64_README_autotoc_md61}
In order to identify the base64 encoded content, the values are marked with the prefix {\ttfamily @BASE64}. To distinguish between the {\ttfamily @} as character and {\ttfamily @} as Base64 marker, all strings starting with {\ttfamily @} will be modified so that they begin with {\ttfamily @@}.\hypertarget{md_src_plugins_base64_README_autotoc_md62}{}\doxyparagraph{Example}\label{md_src_plugins_base64_README_autotoc_md62}
...
\DoxyCodeLine{sudo kdb mount test\_config.toml user:/tests/base64 toml base64}
\DoxyCodeLine{}
...
\DoxyCodeLine{sudo kdb umount user:/tests/base64}

\end{DoxyCode}
\hypertarget{md_src_plugins_base64_README_autotoc_md63}{}\doxysubsubsection{Meta Mode}\label{md_src_plugins_base64_README_autotoc_md63}
...
\hypertarget{md_src_plugins_base64_README_autotoc_md64}{}\doxyparagraph{Example}\label{md_src_plugins_base64_README_autotoc_md64}
...
\DoxyCodeLine{sudo kdb mount test\_config.ni user:/tests/base64 ni base64 binary/meta=}
\DoxyCodeLine{}

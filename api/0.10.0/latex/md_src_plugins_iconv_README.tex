...
This plugin is a filter plugin that converts between different character encodings.\hypertarget{md_src_plugins_iconv_README_autotoc_md271}{}\doxysection{Installation}\label{md_src_plugins_iconv_README_autotoc_md271}
See \mbox{\hyperlink{doc_INSTALL_md}{installation}}. The package is called {\ttfamily libelektra5-\/extra}.\hypertarget{md_src_plugins_iconv_README_autotoc_md272}{}\doxysection{Purpose}\label{md_src_plugins_iconv_README_autotoc_md272}
...
Note that for writing the configuration {\ttfamily from} and {\ttfamily to} is swapped. A key database that requires a specific encoding can make use of it. To sum up, every user can select a different encoding, but the key databases are still properly encoded for anyone.\hypertarget{md_src_plugins_iconv_README_autotoc_md273}{}\doxysection{Example}\label{md_src_plugins_iconv_README_autotoc_md273}

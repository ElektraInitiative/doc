...
\end{DoxyItemize}\hypertarget{doc_BUILDSERVER_md_autotoc_md1544}{}\doxysection{Setup}\label{doc_BUILDSERVER_md_autotoc_md1544}
This section aims to give an introduction into this setup.\hypertarget{doc_BUILDSERVER_md_autotoc_md1545}{}\doxysubsection{Multibranch Pipeline Jobs (libelektra)}\label{doc_BUILDSERVER_md_autotoc_md1545}
...
Summarized libelektra\textquotesingle{}s job purpose is to combine the where (our GIT repository), with a when (tracking changes via polling or webhooks) with a how (pointing to the Jenkinsfile + configuration).\hypertarget{doc_BUILDSERVER_md_autotoc_md1546}{}\doxysubsection{Jenkins Shared Library}\label{doc_BUILDSERVER_md_autotoc_md1546}
...
It is available in the \href{https://github.com/ElektraInitiative/jenkins-library}{\texttt{ following repository}}. For more detailed usage instruction or instructions on how to integrate this shared library into Jenkins, see its \href{https://github.com/ElektraInitiative/jenkins-library/blob/master/README.md}{\texttt{ README.\+md}}.\hypertarget{doc_BUILDSERVER_md_autotoc_md1547}{}\doxysubsection{Jenkinsfiles}\label{doc_BUILDSERVER_md_autotoc_md1547}
Jenkinsfiles describe what actions the build system should execute on which build slave. Currently Elektra uses four different files.\hypertarget{doc_BUILDSERVER_md_autotoc_md1548}{}\doxysubsubsection{Jenkinsfile.\+daily}\label{doc_BUILDSERVER_md_autotoc_md1548}
...
\end{DoxyItemize}\hypertarget{doc_BUILDSERVER_md_autotoc_md1549}{}\doxysubsubsection{Jenkinsfile.\+monthly}\label{doc_BUILDSERVER_md_autotoc_md1549}
...
\end{DoxyItemize}\hypertarget{doc_BUILDSERVER_md_autotoc_md1550}{}\doxysubsubsection{Jenkinsfile.\+release}\label{doc_BUILDSERVER_md_autotoc_md1550}
...
\end{DoxyItemize}\hypertarget{doc_BUILDSERVER_md_autotoc_md1551}{}\doxysubsubsection{Jenkinsfile}\label{doc_BUILDSERVER_md_autotoc_md1551}
...
\end{DoxyItemize}\hypertarget{doc_BUILDSERVER_md_autotoc_md1552}{}\doxysubsubsection{DSL}\label{doc_BUILDSERVER_md_autotoc_md1552}
...
When adding new stages to the build chain it is generally a good idea to look up an existing stage which does something similar and adapt it to the new use case.\hypertarget{doc_BUILDSERVER_md_autotoc_md1553}{}\doxysubsection{Security}\label{doc_BUILDSERVER_md_autotoc_md1553}
Since a malicious PR could easily destroy the build server and slaves or expose credentials some restrictions are introduced. Only PR authors that have the right to push to libelektra can modify the Jenkinsfile and have those changes be respected for the respective branch.\hypertarget{doc_BUILDSERVER_md_autotoc_md1554}{}\doxysubsection{Test Environments}\label{doc_BUILDSERVER_md_autotoc_md1554}
We use Docker containers to provide the various test environments.

They are described \href{https://master.libelektra.org/scripts/docker}{\texttt{ in the repository}} and the Jenkinsfile describes how to build them. If a rebuild of the images is needed is determined by the hash of the Dockerfile used to describe it. If it has not changed the build step will be skipped. In the case that an image needed a build it will afterwards be uploaded into a private Docker image registry (on a7) and thus is shared between all Docker capable build slaves.\hypertarget{doc_BUILDSERVER_md_autotoc_md1555}{}\doxysection{Jenkinsfile (\+Main CI Pipeline)}\label{doc_BUILDSERVER_md_autotoc_md1555}
\hypertarget{doc_BUILDSERVER_md_autotoc_md1556}{}\doxysubsection{Tests}\label{doc_BUILDSERVER_md_autotoc_md1556}
...
Since there is no strict way to enforce the way tests are added we encourage you to read the existing configuration and modify existing tests so they suite your needs.\hypertarget{doc_BUILDSERVER_md_autotoc_md1557}{}\doxysubsection{Deployment}\label{doc_BUILDSERVER_md_autotoc_md1557}
...
Additionally we recompile the homepage and deploy it on the community node.\hypertarget{doc_BUILDSERVER_md_autotoc_md1558}{}\doxysection{Jenkinsfile.\+release (\+Release Pipeline)}\label{doc_BUILDSERVER_md_autotoc_md1558}
\hypertarget{doc_BUILDSERVER_md_autotoc_md1559}{}\doxysubsection{Release Builds}\label{doc_BUILDSERVER_md_autotoc_md1559}
...
The generated packages are additionally installed and tested on a clean Docker image.\hypertarget{doc_BUILDSERVER_md_autotoc_md1560}{}\doxysubsection{Manual approval}\label{doc_BUILDSERVER_md_autotoc_md1560}
In order to verify the correct behavior and results of the previous steps and their generated artifacts, we use the \href{https://www.jenkins.io/doc/pipeline/steps/pipeline-input-step/}{\texttt{ Input Step}} to pause the pipeline. After manually verifying the correctness, the pipeline run can be resumed.\hypertarget{doc_BUILDSERVER_md_autotoc_md1561}{}\doxysubsection{Publishing}\label{doc_BUILDSERVER_md_autotoc_md1561}
The previously generated packages are published to the repositories, as described above in {\itshape Jenkinsfile (Main CI Pipeline)}. The API documentation and source packages get published to the relevant git repositories. Finally the Alpine release image is published to Docker Hub.\hypertarget{doc_BUILDSERVER_md_autotoc_md1562}{}\doxysection{Jenkins Setup}\label{doc_BUILDSERVER_md_autotoc_md1562}
This section describes how to replicate the current Jenkins configuration.\hypertarget{doc_BUILDSERVER_md_autotoc_md1563}{}\doxysubsection{Jenkins libelektra Configuration}\label{doc_BUILDSERVER_md_autotoc_md1563}
...
\end{DoxyItemize}\hypertarget{doc_BUILDSERVER_md_autotoc_md1564}{}\doxysubsection{Jenkins libelektra-\/release Configuration}\label{doc_BUILDSERVER_md_autotoc_md1564}
...
\end{DoxyItemize}\hypertarget{doc_BUILDSERVER_md_autotoc_md1565}{}\doxysubsection{General Set-\/\+Up of a New Pipeline}\label{doc_BUILDSERVER_md_autotoc_md1565}
...
\end{DoxyCode}
\hypertarget{doc_BUILDSERVER_md_autotoc_md1566}{}\doxysubsection{Adding a Jenkins Node}\label{doc_BUILDSERVER_md_autotoc_md1566}
...
All files and folders in the Node under {\ttfamily /home/jenkins} should be owned by user {\ttfamily jenkins}.\hypertarget{doc_BUILDSERVER_md_autotoc_md1567}{}\doxysection{Understanding Jenkins Output}\label{doc_BUILDSERVER_md_autotoc_md1567}
Our Jenkins build uses parallel steps inside a single build job to do most of the work. To reliable determine which stages failed it is best to look over the build results in the Jenkins Blue Ocean view. It is the default View opened when accessing the build results from Git\+Hub. For libelektra the URLs are \href{https://build.libelektra.org/job/libelektra/}{\texttt{ https\+://build.\+libelektra.\+org/job/libelektra/}} and \href{https://build.libelektra.org/blue/organizations/jenkins/libelektra/branches/}{\texttt{ https\+://build.\+libelektra.\+org/blue/organizations/jenkins/libelektra/branches/}} Failed stages are marked in red. On selecting a stage you can further expand all steps in the stage. Of interest should be one of the first steps which echos out the Docker image used to run the stage. You also want to look for whatever failed (which should be in a step also marked red to indicate failure).\hypertarget{doc_BUILDSERVER_md_autotoc_md1568}{}\doxysection{Reproducing Build Server Errors Locally}\label{doc_BUILDSERVER_md_autotoc_md1568}
...
Alternatively, you can follow the \mbox{\hyperlink{doc_tutorials_run_all_tests_with_docker_md}{tutorial}}.\hypertarget{doc_BUILDSERVER_md_autotoc_md1569}{}\doxysection{Modify Test Environments}\label{doc_BUILDSERVER_md_autotoc_md1569}
...
Additionally {\ttfamily jenkins build all please} can be used to trigger all build jobs relevant for PR\textquotesingle{}s. This is not necessary anymore as all relevant tests have been moved into the libelektra job.\hypertarget{doc_BUILDSERVER_md_autotoc_md1571}{}\doxysection{Authorization}\label{doc_BUILDSERVER_md_autotoc_md1571}
...
\end{DoxyVerb}
 \hypertarget{doc_BUILDSERVER_md_autotoc_md1572}{}\doxysection{Issues with the Build Environment}\label{doc_BUILDSERVER_md_autotoc_md1572}

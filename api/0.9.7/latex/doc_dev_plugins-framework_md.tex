...
We see in the listing above that the plugin generates and returns the contract if, and only if, the name of the {\ttfamily parent\+Key} is {\ttfamily system\+:/elektra/modules/ccode}. The user and the contract checker can access the contract of ccode below the key {\ttfamily system\+:/elektra/modules/ccode} in the same way other configuration is accessed. Note that we also have to {\ttfamily return 1} at the end of the contract to not execute the regular functionality of the plugin.\hypertarget{doc_dev_plugins-framework_md_autotoc_md2201}{}\doxysection{Exports}\label{doc_dev_plugins-framework_md_autotoc_md2201}
...
If you want to use an exported function from a symbol, please look at \href{/home/jenkins/workspace/libelektra-release/src/libs/tools/src/plugin.cpp}{\texttt{ Plugin\+::parse}}.\hypertarget{doc_dev_plugins-framework_md_autotoc_md2202}{}\doxysection{Changing Plugins}\label{doc_dev_plugins-framework_md_autotoc_md2202}
...
Plugins can also be viewed as framework extensions. A component abstracts plugins. But this term is misleading in our case, because components usually can choose which interfaces they implement. Elektra’s plugins, however, are restricted to implement one specific interface. Without contracts, plugins could not interact as described in this chapter.\hypertarget{doc_dev_plugins-framework_md_autotoc_md2203}{}\doxysection{SEE ALSO}\label{doc_dev_plugins-framework_md_autotoc_md2203}

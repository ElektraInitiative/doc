...
The {\ttfamily hosts} plugins transforms the information of this file to the following structure. The keys directly below {\ttfamily ipv4} or {\ttfamily ipv6} are host names of IPv4 or IPv6 addresses, respectively. The keys directly below these keys are aliases. The IP addresses themselves are stored as values.\hypertarget{md_src_plugins_hosts_README_autotoc_md244}{}\doxysection{Special values}\label{md_src_plugins_hosts_README_autotoc_md244}
\hypertarget{md_src_plugins_hosts_README_autotoc_md245}{}\doxysubsection{Hostnames}\label{md_src_plugins_hosts_README_autotoc_md245}
Canonical hostnames are stored as key base names with their IP addresses as value.\hypertarget{md_src_plugins_hosts_README_autotoc_md246}{}\doxysubsection{Aliases}\label{md_src_plugins_hosts_README_autotoc_md246}
Aliases are stored as keys directly below canonical hostnames with a read-\/only duplicate of the associated IP address as value.\hypertarget{md_src_plugins_hosts_README_autotoc_md247}{}\doxysubsection{Comments}\label{md_src_plugins_hosts_README_autotoc_md247}
Comments are stored according to the comment metadata specification (see \href{/home/jenkins/workspace/libelektra-release/doc/METADATA.ini}{\texttt{ /doc/\+METADATA.ini}} for more information).\hypertarget{md_src_plugins_hosts_README_autotoc_md248}{}\doxysubsection{Ordering}\label{md_src_plugins_hosts_README_autotoc_md248}
The ordering of the hosts is stored in metakeys of type {\ttfamily order}. The value is an ascending number. Ordering of aliases is {\itshape not} preserved.\hypertarget{md_src_plugins_hosts_README_autotoc_md249}{}\doxysection{Examples}\label{md_src_plugins_hosts_README_autotoc_md249}
Mount the plugin\+:
...
\DoxyCodeLine{echo '127.0.0.1 localhost' >  `kdb file /tests/hosts`}
\DoxyCodeLine{echo '::1       localhost'          >> `kdb file /tests/hosts`}
\DoxyCodeLine{}
...
\DoxyCodeLine{\#> ::1}
\DoxyCodeLine{}
\DoxyCodeLine{\# Should both fail with error 04200 and return 5}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_hosts_README_autotoc_md250}{}\doxysection{Limitations}\label{md_src_plugins_hosts_README_autotoc_md250}

...
\Hypertarget{doc_tutorials_run_all_tests_with_docker_md_md_doc_tutorials_run_all_tests_with_docker}%
 Running all the tests like the build server requires multiple dependencies. To overcome this problem, instead of trying to install all the necessary dependencies on your own, an appropriate Docker image can be used. This way you can easily and quickly run all the tests.\hypertarget{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4024}{}\doxysection{Who Is This Guide For?}\label{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4024}
For anyone who wants to run all the tests, like it is done by the build server.

This is a step-\/by-\/step guide. Just follow the steps and you are good to go!\hypertarget{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4025}{}\doxysection{Prerequisites}\label{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4025}

\begin{DoxyItemize}
\item Docker for Linux containers has to be pre-\/installed. Please refer to \href{https://docs.docker.com/install/}{\texttt{ https\+://docs.\+docker.\+com/install/}} if you haven\textquotesingle{}t installed it yet. Your host OS can be either Linux, mac\+OS or Windows. Alternatively, you can use podman, a different container engine which is compatible with Docker. See \href{https://podman.io/}{\texttt{ https\+://podman.\+io/}} for more details and an installation guide.
\item Basic knowledge of Docker (not mandatory)
\end{DoxyItemize}\hypertarget{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4026}{}\doxysection{What to Begin With?}\label{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4026}
\hypertarget{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4027}{}\doxysubsection{1. Docker Image}\label{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4027}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{podman build -\/t buildelektra-\/sid \(\backslash\)}
\DoxyCodeLine{        -\/-\/build-\/arg JENKINS\_USERID=\$(id -\/u) \(\backslash\)}
\DoxyCodeLine{        -\/-\/build-\/arg JENKINS\_GROUPID=\$(id -\/g) \(\backslash\)}
\DoxyCodeLine{        -\/f scripts/docker/debian/sid/Dockerfile \(\backslash\)}
\DoxyCodeLine{        scripts/docker/debian/sid/}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{podman run -\/-\/rm anoxis/registry-\/cli -\/r https://hub-\/public.libelektra.org}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{podman pull hub-\/public.libelektra.org/<image\_name>:<tag\_name>}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{podman pull hub-\/public.libelektra.org/build-\/elektra-\/debian-\/stretch:202108-\/1c5cb52603c30b89b7c3b26234cb7094f03f180ea5378b8e349b2feee9a9d724}

\end{DoxyCode}
\hypertarget{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4028}{}\doxysubsection{2. Run the Docker Container}\label{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4028}
...
\end{DoxyCode}
\hypertarget{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4029}{}\doxysubsubsection{2.\+a Run the Docker Container with podman}\label{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4029}
...
With {\ttfamily podman unshare ls -\/al} you can see the currently set owner and permissions from the container\textquotesingle{}s perspective.

The output inside the root source directory might look something like this\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   424  3. Nov 13:08 .}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root    20  3. Nov 13:08 ..}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   238  3. Nov 13:08 benchmarks}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root 10522  3. Nov 13:08 .cirrus.yml}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root  1479  3. Nov 13:08 .clang-\/format}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root  1621  3. Nov 13:08 .cmake-\/format.yaml}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root  3991  3. Nov 13:08 CMakeLists.txt}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root  3171  3. Nov 13:08 CODE\_OF\_CONDUCT.md}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root    25  3. Nov 13:08 .coveralls.yml}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   692  3. Nov 13:08 doc}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   888  3. Nov 13:08 examples}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   138  3. Nov 13:08 .git}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   148  3. Nov 13:08 .github}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root  1422  3. Nov 13:08 .gitignore}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   132  3. Nov 13:08 .idea}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root   558  3. Nov 13:08 lgtm.yml}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root  1530  3. Nov 13:08 LICENSE.md}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   134  3. Nov 13:08 LICENSES}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root   142  3. Nov 13:08 .oclint}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root  9714  3. Nov 13:08 README.md}
\DoxyCodeLine{-\/rw-\/r-\/-\/r-\/-\/. 1 root root   643  3. Nov 13:08 .restyled.yaml}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root     8  3. Nov 13:08 .reuse}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   956  3. Nov 13:08 scripts}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   110  3. Nov 13:08 src}
\DoxyCodeLine{drwxr-\/xr-\/x. 1 root root   316  3. Nov 13:08 tests}

\end{DoxyCode}


Inside the source directory, you can change the permissions to any user id with {\ttfamily podman unshare chown 1000\+:1000 -\/R .}. Keep in mind that this changes the {\itshape host} filesystem. You can read more about this \href{https://docs.podman.io/en/latest/markdown/podman-run.1.html}{\texttt{ here}}.

Finally, you can run the container with\+:


\begin{DoxyCode}{0}
\DoxyCodeLine{podman run -\/-\/user 1000 -\/it -\/-\/rm \(\backslash\)}
\DoxyCodeLine{-\/v "{}\$PWD:/home/jenkins/workspace:Z"{} \(\backslash\)}
\DoxyCodeLine{-\/w /home/jenkins/workspace buildelektra-\/sid}

\end{DoxyCode}


Do not forget the {\ttfamily \+:Z} label. You can read more about the labels in the podman \href{https://docs.podman.io/en/latest/index.html}{\texttt{ documentation}}.\hypertarget{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4030}{}\doxysubsection{3. Build}\label{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4030}
...
\end{DoxyCode}
\hypertarget{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4031}{}\doxysubsection{4. Run Tests}\label{doc_tutorials_run_all_tests_with_docker_md_autotoc_md4031}

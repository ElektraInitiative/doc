...
The implementation adds no significant memory overhead per {\ttfamily Key} if no metadata is used. For embedded systems it is useful to have keys without metadata. Special plugins can help for systems that have very limited memory capacity. Also for systems with enough memory we should consider that adding the first metadata to a key has some additional overhead. In the current implementation a new {\ttfamily Key\+Set} is allocated in this situation.\hypertarget{doc_dev_metadata_md_autotoc_md2674}{}\doxysubsection{Interface}\label{doc_dev_metadata_md_autotoc_md2674}
...
Developers used to Elektra will immediately be familiar with the interface. Tiny wrapper functions still support the old metadata interface.\hypertarget{doc_dev_metadata_md_autotoc_md2675}{}\doxysubsection{Sharing of Metakey}\label{doc_dev_metadata_md_autotoc_md2675}

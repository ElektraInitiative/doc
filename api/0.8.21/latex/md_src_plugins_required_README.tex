...
If you used \hyperlink{md_src_plugins_simplespeclang_README_src_plugins_simplespeclang_README_md}{simplespeclang} and want to only allow keys that are present in the specification, you can add {\ttfamily required} to the {\ttfamily spec-\/mount} command\+: \begin{DoxyVerb}% kdb mount test.spec spec/test simplespeclang
...
% kdb set /test/rejected  something  # fails because rejected not required\end{DoxyVerb}

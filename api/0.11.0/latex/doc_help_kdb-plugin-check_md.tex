...
\Hypertarget{doc_help_kdb-plugin-check_md_md_doc_help_kdb_plugin_check}%
 \hypertarget{doc_help_kdb-plugin-check_md_autotoc_md3048}{}\doxysection{SYNOPSIS}\label{doc_help_kdb-plugin-check_md_autotoc_md3048}
{\ttfamily kdb plugin-\/check \mbox{[}$<$plugin$>$\mbox{]}}\hypertarget{doc_help_kdb-plugin-check_md_autotoc_md3049}{}\doxysection{DESCRIPTION}\label{doc_help_kdb-plugin-check_md_autotoc_md3049}
This command is used to perform checks on the key database or an Elektra plugin.

Where the option argument, {\ttfamily plugin} is the plugin that a user wants to check. Use {\ttfamily -\/c} to pass options to that plugin. If no {\ttfamily plugin} argument is provided a check will be performed on the key database itself. Special values are returned upon exit to represent the outcome of a check.\hypertarget{doc_help_kdb-plugin-check_md_autotoc_md3050}{}\doxysection{OPTIONS}\label{doc_help_kdb-plugin-check_md_autotoc_md3050}
...
\end{DoxyItemize}\hypertarget{doc_help_kdb-plugin-check_md_autotoc_md3051}{}\doxysection{RETURN VALUES}\label{doc_help_kdb-plugin-check_md_autotoc_md3051}
...
\item 11\+: No such plugin found or plugin could not be opened.
\item 12\+: Plugin did not pass checks.
\item 13\+: Plugin has warnings.
\end{DoxyItemize}
...
Since the error code is a return value, it is not automatically displayed to the shell. If the user wants to have the value printed, they must do so manually (by running a command such as {\ttfamily echo \$?}).\hypertarget{doc_help_kdb-plugin-check_md_autotoc_md3052}{}\doxysection{EXAMPLES}\label{doc_help_kdb-plugin-check_md_autotoc_md3052}
...
To check the {\ttfamily line} plugin\+:~\newline
 {\ttfamily kdb plugin-\/check line}\hypertarget{doc_help_kdb-plugin-check_md_autotoc_md3053}{}\doxysection{SEE ALSO}\label{doc_help_kdb-plugin-check_md_autotoc_md3053}

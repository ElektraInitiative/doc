...
The format is also useful for I\+PC and streaming, because of this it is used by the {\ttfamily specload} plugin.\hypertarget{md_src_plugins_quickdump_README_autotoc_md563}{}\doxysection{Format}\label{md_src_plugins_quickdump_README_autotoc_md563}
...
To distinguish between binary and string keys the (length of the) key value is prefixed with either a {\ttfamily b} or an {\ttfamily s}. Each metakey is prefixed with an {\ttfamily m}, unless we detect that the same metakey was already present on a previous key (e.\+g. through {\ttfamily key\+Copy\+Meta}). In this case the prefix {\ttfamily c} is used and instead of the metakey name and value, we write the name of the previous key and the metakey name.\hypertarget{md_src_plugins_quickdump_README_autotoc_md564}{}\doxysubsection{Variable Length Integer encoding}\label{md_src_plugins_quickdump_README_autotoc_md564}
...
Thanks to \href{https://github.com/stoklund/varint}{\texttt{ https\+://github.\+com/stoklund/varint}} for listing various integer encodings.\hypertarget{md_src_plugins_quickdump_README_autotoc_md565}{}\doxysection{Old Formats}\label{md_src_plugins_quickdump_README_autotoc_md565}
All old versions can still be read by this plugin, but we will always write the newest format.\hypertarget{md_src_plugins_quickdump_README_autotoc_md566}{}\doxysubsection{Version 1}\label{md_src_plugins_quickdump_README_autotoc_md566}
The first version used the magic number {\ttfamily 0x454b444200000001} and stored the full keynames, instead of one relative to the parent key.\hypertarget{md_src_plugins_quickdump_README_autotoc_md567}{}\doxysubsection{Version 2}\label{md_src_plugins_quickdump_README_autotoc_md567}
The second version used the magic number {\ttfamily 0x454b444200000001} and always used 64-\/bit integers to store the length of strings.\hypertarget{md_src_plugins_quickdump_README_autotoc_md568}{}\doxysection{Usage}\label{md_src_plugins_quickdump_README_autotoc_md568}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_quickdump_README_autotoc_md569}{}\doxysection{Dependencies}\label{md_src_plugins_quickdump_README_autotoc_md569}
None.\hypertarget{md_src_plugins_quickdump_README_autotoc_md570}{}\doxysection{Examples}\label{md_src_plugins_quickdump_README_autotoc_md570}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_quickdump_README_autotoc_md571}{}\doxysection{Limitations}\label{md_src_plugins_quickdump_README_autotoc_md571}
None. 
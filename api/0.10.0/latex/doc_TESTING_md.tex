...
\Hypertarget{doc_TESTING_md_md_doc_TESTING}%
 \hypertarget{doc_TESTING_md_autotoc_md4248}{}\doxysection{Introduction}\label{doc_TESTING_md_autotoc_md4248}
Libraries need pervasive testing for continuous improvement. Any problem found and behavior described must be written down as test so that it is assured that no new regressions will be added.\hypertarget{doc_TESTING_md_autotoc_md4249}{}\doxysection{Running Tests}\label{doc_TESTING_md_autotoc_md4249}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4250}{}\doxysection{Required Environment}\label{doc_TESTING_md_autotoc_md4250}
...
\end{DoxyEnumerate}\hypertarget{doc_TESTING_md_autotoc_md4251}{}\doxysection{Manual Testing}\label{doc_TESTING_md_autotoc_md4251}
...
Pay attention that sourcing depends on the operating system or rather the shell. For example on standard Free\+BSD 11.\+3 you have to execute {\ttfamily sh} in the root of the repository first. Then do {\itshape not} use the {\ttfamily source} command but the point {\ttfamily .} as explained above.\hypertarget{doc_TESTING_md_autotoc_md4252}{}\doxysection{Using debuggers}\label{doc_TESTING_md_autotoc_md4252}
...
You can also use Docker to set up such an environment. There is \mbox{\hyperlink{doc_tutorials_run_all_tests_with_docker_md}{a tutorial}} that guides you through the necessary steps.\hypertarget{doc_TESTING_md_autotoc_md4254}{}\doxysection{Adding Tests}\label{doc_TESTING_md_autotoc_md4254}
...
By using {\ttfamily TEST\+\_\+\+README} in {\ttfamily add\+\_\+plugin} (also enabled by default), \href{https://master.libelektra.org/tests/shell/shell_recorder/tutorial_wrapper}{\texttt{ Markdown Shell Recorder}} are expected to be in the README.\+md of the plugin.\hypertarget{doc_TESTING_md_autotoc_md4255}{}\doxysection{Conventions}\label{doc_TESTING_md_autotoc_md4255}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4256}{}\doxysection{Strategy}\label{doc_TESTING_md_autotoc_md4256}
The testing must happen on every level of the software to achieve a maximum coverage with the available time. In the rest of the document we describe the different levels and where these tests are.\hypertarget{doc_TESTING_md_autotoc_md4257}{}\doxysubsection{CFramework}\label{doc_TESTING_md_autotoc_md4257}
This is basically a bunch of assertion macros and some output facilities. It is written in pure C and very lightweight.

It is located here.\hypertarget{doc_TESTING_md_autotoc_md4258}{}\doxysubsection{ABI Tests}\label{doc_TESTING_md_autotoc_md4258}
...
The tests are located here.\hypertarget{doc_TESTING_md_autotoc_md4259}{}\doxysubsection{C Unit Tests}\label{doc_TESTING_md_autotoc_md4259}
...
They are located here.\hypertarget{doc_TESTING_md_autotoc_md4260}{}\doxysubsubsection{Internal Functions}\label{doc_TESTING_md_autotoc_md4260}
...
get exported. Functions not starting with this prefix are internal only and therefore not visible in the test cases. Test internal functionality by including the corresponding C file.\hypertarget{doc_TESTING_md_autotoc_md4261}{}\doxysubsection{Module Tests}\label{doc_TESTING_md_autotoc_md4261}
...
Use the CMake macro {\ttfamily add\+\_\+plugintest} for adding these tests.\hypertarget{doc_TESTING_md_autotoc_md4262}{}\doxysubsection{C++ Unit Tests}\label{doc_TESTING_md_autotoc_md4262}
C++ Unit tests are done using the Google Test framework. See \mbox{\hyperlink{doc_decisions_6_implemented_unit_testing_md}{architectural decision}}.

Use the CMake macro {\ttfamily add\+\_\+gtest} for adding these tests.\hypertarget{doc_TESTING_md_autotoc_md4263}{}\doxysubsection{Script Tests}\label{doc_TESTING_md_autotoc_md4263}
Tests which need scripts are done using shell recorder or directly with POSIX shell commands. See \mbox{\hyperlink{doc_decisions_6_implemented_script_testing_md}{architectural decision}}.
...
See here.\hypertarget{doc_TESTING_md_autotoc_md4264}{}\doxysubsection{Shell Recorder}\label{doc_TESTING_md_autotoc_md4264}
The more elegant way to specify script tests are via the so-\/called Shell Recorder using Markdown Syntax.

See here.\hypertarget{doc_TESTING_md_autotoc_md4265}{}\doxysubsection{Fuzz Testing}\label{doc_TESTING_md_autotoc_md4265}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4266}{}\doxysubsection{ASAN}\label{doc_TESTING_md_autotoc_md4266}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4267}{}\doxysubsubsection{mac\+OS}\label{doc_TESTING_md_autotoc_md4267}
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4268}{}\doxysubsection{CBMC}\label{doc_TESTING_md_autotoc_md4268}
For bounded model checking tests, see {\ttfamily scripts/cbmc}.\hypertarget{doc_TESTING_md_autotoc_md4269}{}\doxysubsection{Static Code Checkers}\label{doc_TESTING_md_autotoc_md4269}
There is a number of static code checkers available for all kind of programming languages. The following section show how the most common ones can be used with {\ttfamily libelektra}.\hypertarget{doc_TESTING_md_autotoc_md4270}{}\doxysubsubsection{Cppcheck}\label{doc_TESTING_md_autotoc_md4270}
\href{https://cppcheck.sourceforge.net/}{\texttt{ Cppcheck}} can be used directly on a C or C++ source file by calling it with {\ttfamily cppcheck -\/-\/enable=all $<$sourcefile$>$}. This way it might miss some header files though and thus doesn\textquotesingle{}t detect all possible issues, but still gives useful hints in general.

To analyze the whole project, use it in conjunction with CMake by calling {\ttfamily cmake} with the parameter {\ttfamily -\/DCMAKE\+\_\+\+EXPORT\+\_\+\+COMPILE\+\_\+\+COMMANDS=ON}. This way CMake creates a file called {\ttfamily compile\+\_\+commands.\+json} in the build directory. Afterwards, call {\ttfamily cppcheck} with the CMake settings and store the output as xml\+:
...
Now you can view the html report by opening {\ttfamily index.\+html} in the specified folder to get an overview of the issues found in the whole project.\hypertarget{doc_TESTING_md_autotoc_md4271}{}\doxysubsubsection{OCLint}\label{doc_TESTING_md_autotoc_md4271}
\href{https://oclint.org/}{\texttt{ OCLint}} is a static code analyzer for C, C++ and Objective C. To use this tool enable the CMake option {\ttfamily CMAKE\+\_\+\+EXPORT\+\_\+\+COMPILE\+\_\+\+COMMANDS}. The steps below show a step-\/by-\/step guide on how to analyze files with OCLint.
...
\end{DoxyCode}
\hypertarget{doc_TESTING_md_autotoc_md4272}{}\doxysubsubsection{scan-\/build}\label{doc_TESTING_md_autotoc_md4272}
...
Alternatively, you can also open the {\ttfamily index.\+html} file in the aforementioned folder, but using the tool the report is enriched with further information.\hypertarget{doc_TESTING_md_autotoc_md4273}{}\doxysubsubsection{Sonar\+Lint}\label{doc_TESTING_md_autotoc_md4273}
\href{https://www.sonarlint.org/}{\texttt{ Sonar\+Lint}} is a static code checker primarily intended for Java. It is usually used by installing the corresponding plugin for the used IDE, then there is no further configuration required.\hypertarget{doc_TESTING_md_autotoc_md4274}{}\doxysubsection{Randoop}\label{doc_TESTING_md_autotoc_md4274}
For using the unit test generator randoop with the jna bindings, see {\ttfamily scripts/randoop/randoop}.\hypertarget{doc_TESTING_md_autotoc_md4275}{}\doxysubsection{Code Coverage}\label{doc_TESTING_md_autotoc_md4275}
Run\+:
...
The HTML files can be found in the build directory in the folder {\ttfamily coverage}.\hypertarget{doc_TESTING_md_autotoc_md4276}{}\doxysection{See Also}\label{doc_TESTING_md_autotoc_md4276}

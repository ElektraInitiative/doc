...
Hidden keys are not useful for Elektra. Instead comments or other metadata contain information about keys that is not considered to belong to the configuration. If hidden keys are desired, we can still write a plugin to filter specific keys out.\hypertarget{doc_help_elektra-semantics_md_autotoc_md2278}{}\doxysection{Problems}\label{doc_help_elektra-semantics_md_autotoc_md2278}
This section explains why using file system semantics for configuration is not a good idea.\hypertarget{doc_help_elektra-semantics_md_autotoc_md2279}{}\doxysubsection{filesys}\label{doc_help_elektra-semantics_md_autotoc_md2279}
...
Elektra 0.\+7 already supported more than one backend, but {\ttfamily filesys} was the only backend implementing the full semantics.\hypertarget{doc_help_elektra-semantics_md_autotoc_md2280}{}\doxysubsection{Limitations of File Systems}\label{doc_help_elektra-semantics_md_autotoc_md2280}

...
The O\+P\+M\+P\+HM is not an ordinary hash map. The O\+P\+M\+P\+HM inserts all elements at once, this process is known as build. The O\+P\+M\+P\+HM is designed to return an index when looking up an element. Those indices must be known when building the O\+P\+M\+P\+HM.

The desired return index, also known as the order is set in {\ttfamily Opmphm\+Graph-\/$>$edges\mbox{[}i\mbox{]}.order}. Where {\ttfamily i} is the i-\/th element.

The build consists of three steps.

\subsubsection*{Initialization}

Use {\ttfamily opmphm\+New ()} and {\ttfamily opmphm\+Graph\+New (...)} to instantiate the needed structures. The function {\ttfamily opmphm\+Graph\+New (...)} takes a parameter {\ttfamily c}, use the {\ttfamily opmphm\+MinC ()} to ensure the the passed value is over the minimum. To initialize the O\+P\+M\+P\+HM build the {\ttfamily Opmphm\+Init} must be set with information about your data. Set your data array {\ttfamily Opmphm\+Init-\/$>$data} and the string extraction function {\ttfamily Opmphm\+Init-\/$>$get\+String}, which should extract the string from a single data array entry. Provide a good seed in {\ttfamily Opmphm\+Init-\/$>$init\+Seed}, needed in the next step.

\subsubsection*{Mapping}

The function {\ttfamily opmphm\+Mapping} uses your seed (the {\ttfamily Opmphm\+Init-\/$>$seed} will be changed) and tries to map your elements to edges in a acyclic r-\/partite hypergraph, this mapping might not succeed, on cycles just call it again. When the {\ttfamily c} value from the initialization is lower than the minimum, the probability of finding such a acyclic r-\/partite hypergraph is 0.

...
And even more\+: 
\begin{DoxyCode}
1 Key *k1 = keyNew("user/a", KEY\_OWNER, "markus", KEY\_END);
2 Key *k2 = keyNew("user/a", KEY\_OWNER, "max", KEY\_END);
3 
4 // keyCmp(k1,k2) < 0
5 // keyCmp(k2,k1) > 0
\end{DoxyCode}
...
\begin{DoxyRetVals}{Return values}
{\em 1} & if key name begins with {\ttfamily system}, 0 otherwise \\
\hline
{\em -\/1} & on N\+U\+L\+L pointer \\
\hline
\end{DoxyRetVals}
...
\begin{DoxyRetVals}{Return values}
{\em 1} & if key name begins with {\ttfamily user}, 0 otherwise \\
\hline
{\em -\/1} & on N\+U\+L\+L pointer \\
\hline
\end{DoxyRetVals}
...
3 succeed\_if (keyRel (key, check) == 0, "should be same");
\end{DoxyCode}

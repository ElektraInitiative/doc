...
\Hypertarget{doc_GETSTARTED_md_md_doc_GETSTARTED}%
 This document is intended for developers who want to get started with developing with Elektra.\hypertarget{doc_GETSTARTED_md_autotoc_md2689}{}\doxysection{Skill requirements}\label{doc_GETSTARTED_md_autotoc_md2689}

\begin{DoxyItemize}
\item Operating system

We recommend a Unix-\/based operating system to run Elektra (Linux, BSD, mac\+OS) but \href{https://www.libelektra.org/tutorials/contributing-with-visual-studio-windows}{\texttt{ it\textquotesingle{}s also possible to use Windows which is supported but not yet fully tested}}.
\item Using command-\/line interface and commands

The easiest way to compile, install and use Elektra is by using the terminal. We will introduce the basic commands which you will need to run Elektra for the very first time. \href{https://www.libelektra.org/tutorials/contributing-with-clion}{\texttt{ It\textquotesingle{}s also possible to use CLion}}
\item Basic knowledge about git

\href{https://git-scm.com/}{\texttt{ Git}} is a distributed version control system to track changes of the source code in a project. We will use a single Git command to get the source code of Elektra.
\item Basic knowledge about make/\+CMake

Don\textquotesingle{}t panic! \href{https://www.gnu.org/software/make/}{\texttt{ make}} or \href{https://cmake.org/}{\texttt{ CMake}} are used to generate an executable program from the code. If you are not used to these tools, it\textquotesingle{}s not a problem, we will introduce them to you in later sections.
...
\end{DoxyItemize}\hypertarget{doc_GETSTARTED_md_autotoc_md2690}{}\doxysection{Software requirements}\label{doc_GETSTARTED_md_autotoc_md2690}
...
\end{DoxyCode}
\hypertarget{doc_GETSTARTED_md_autotoc_md2691}{}\doxysection{Installation}\label{doc_GETSTARTED_md_autotoc_md2691}
...
\end{DoxyCode}
\hypertarget{doc_GETSTARTED_md_autotoc_md2692}{}\doxysection{Hello first steps!}\label{doc_GETSTARTED_md_autotoc_md2692}
...
Modify the \href{https://www.libelektra.org/home}{\texttt{ website}} with the following tutorial. The website is build with angular and hosted with grunt.

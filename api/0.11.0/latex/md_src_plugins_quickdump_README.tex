...
\item infos/status = maintained compatible unittest tested nodep libc configurable preview
\item infos/metadata =
...
The format is also useful for IPC and streaming, because of this it is used by the {\ttfamily specload} plugin.\hypertarget{md_src_plugins_quickdump_README_autotoc_md445}{}\doxysection{Format}\label{md_src_plugins_quickdump_README_autotoc_md445}
...
To distinguish between binary and string keys the (length of the) key value is prefixed with either a {\ttfamily b} or an {\ttfamily s}. Each metakey is prefixed with an {\ttfamily m}, unless we detect that the same metakey was already present on a previous key (e.\+g. through {\ttfamily key\+Copy\+Meta}). In this case the prefix {\ttfamily c} is used and instead of the metakey name and value, we write the name of the previous key and the metakey name.\hypertarget{md_src_plugins_quickdump_README_autotoc_md446}{}\doxysubsection{Variable Length Integer encoding}\label{md_src_plugins_quickdump_README_autotoc_md446}
...
Thanks to \href{https://github.com/stoklund/varint}{\texttt{ https\+://github.\+com/stoklund/varint}} for listing various integer encodings.\hypertarget{md_src_plugins_quickdump_README_autotoc_md447}{}\doxysection{Usage}\label{md_src_plugins_quickdump_README_autotoc_md447}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_quickdump_README_autotoc_md448}{}\doxysection{Dependencies}\label{md_src_plugins_quickdump_README_autotoc_md448}
None.\hypertarget{md_src_plugins_quickdump_README_autotoc_md449}{}\doxysection{Examples}\label{md_src_plugins_quickdump_README_autotoc_md449}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_quickdump_README_autotoc_md450}{}\doxysection{Limitations}\label{md_src_plugins_quickdump_README_autotoc_md450}
None. 
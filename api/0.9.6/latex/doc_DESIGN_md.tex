...
The C-\/\+API is suitable to be reimplemented, also in non-\/\+C-\/languages, like Rust. Elektra provides a full blown architecture to support configuring systems, and the C-\/\+API is the core of this endeavour.\hypertarget{doc_DESIGN_md_autotoc_md2077}{}\doxysection{Data Structures}\label{doc_DESIGN_md_autotoc_md2077}
The {\ttfamily Key}, {\ttfamily Key\+Set} and {\ttfamily KDB} data structures are defined in {\ttfamily \mbox{\hyperlink{kdbprivate_8h}{kdbprivate.\+h}}} to allow ABI compatibility. This means, it is not possible to put one of Elektra’s data structures on the stack. You must use the memory management facilities mentioned in the next section.\hypertarget{doc_DESIGN_md_autotoc_md2078}{}\doxysection{Memory Management}\label{doc_DESIGN_md_autotoc_md2078}
...
writes the comment in a buffer maintained by you.\hypertarget{doc_DESIGN_md_autotoc_md2079}{}\doxysection{Variable Arguments}\label{doc_DESIGN_md_autotoc_md2079}
...
Alternatively pass a list as described in the documentation. The idea of these variable arguments is, that one function call can create any {\ttfamily Key\+Set}. For binding writers {\ttfamily key\+VNew} might be useful.\hypertarget{doc_DESIGN_md_autotoc_md2080}{}\doxysection{Off-\/by-\/one}\label{doc_DESIGN_md_autotoc_md2080}
...
The same is true for {\ttfamily elektra\+Str\+Len} which also already has the null byte included.\hypertarget{doc_DESIGN_md_autotoc_md2081}{}\doxysection{Minimal Set}\label{doc_DESIGN_md_autotoc_md2081}
...
Useful extensions are available in \mbox{\hyperlink{src_libs_README_md}{further libraries}}.\hypertarget{doc_DESIGN_md_autotoc_md2082}{}\doxysection{Value, String or Binary}\label{doc_DESIGN_md_autotoc_md2082}
...
sets the binary data which might contain `'\textbackslash{}0\textquotesingle{}{\ttfamily . The length is given by}data\+Size\`{}.\hypertarget{doc_DESIGN_md_autotoc_md2083}{}\doxysection{Return Values}\label{doc_DESIGN_md_autotoc_md2083}
...
Elektra does not set {\ttfamily errno}. If a function you call sets {\ttfamily errno}, make sure to set it back to the old value again.\hypertarget{doc_DESIGN_md_autotoc_md2085}{}\doxysection{Naming}\label{doc_DESIGN_md_autotoc_md2085}
...
Function names not belonging to one of the three classes use the prefix {\ttfamily elektra$\ast$}.\hypertarget{doc_DESIGN_md_autotoc_md2086}{}\doxysection{const}\label{doc_DESIGN_md_autotoc_md2086}
...
The reason behind this is, that the above functions – as their name suggest – only retrieve values. The returned value must not be modified directly.\hypertarget{doc_DESIGN_md_autotoc_md2087}{}\doxysection{Design Guidelines Checklist}\label{doc_DESIGN_md_autotoc_md2087}
On potential changes of the API/\+ABI as detected by the \href{https://build.libelektra.org/job/libelektra/job/master/}{\texttt{ {\ttfamily build server}}}, please make sure the API has been reviewed according to the following 2 checklists\+:\hypertarget{doc_DESIGN_md_autotoc_md2088}{}\doxysection{Checklist for overall API}\label{doc_DESIGN_md_autotoc_md2088}
\hypertarget{doc_DESIGN_md_autotoc_md2089}{}\doxysubsection{Consistency}\label{doc_DESIGN_md_autotoc_md2089}
...
\end{DoxyItemize}\hypertarget{doc_DESIGN_md_autotoc_md2090}{}\doxysubsection{Structural Clarity}\label{doc_DESIGN_md_autotoc_md2090}

\begin{DoxyItemize}
\item \mbox{[} \mbox{]} Functions with similar functionality have the same prefix
\end{DoxyItemize}\hypertarget{doc_DESIGN_md_autotoc_md2091}{}\doxysubsection{Compatibility}\label{doc_DESIGN_md_autotoc_md2091}

\begin{DoxyItemize}
\item \mbox{[} \mbox{]} All bindings have been updated to reflect the new API and work properly
\end{DoxyItemize}\hypertarget{doc_DESIGN_md_autotoc_md2092}{}\doxysubsection{Extensibility}\label{doc_DESIGN_md_autotoc_md2092}
...
\end{DoxyItemize}\hypertarget{doc_DESIGN_md_autotoc_md2093}{}\doxysection{Checklist for each function}\label{doc_DESIGN_md_autotoc_md2093}

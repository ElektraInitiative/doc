...
Thus for full and static build variants an exchange at run-\/time is not possible. Using shared libraries, however, {\ttfamily KDB\+\_\+\+STORAGE} and {\ttfamily KDB\+\_\+\+RESOLVER} are actually symbolic links ({\ttfamily libelektra-\/plugin-\/resolver.\+so} and {\ttfamily libelektra-\/plugin-\/storage.\+so}) to concrete plugins and thus can be changed without recompilation.

The {\bfseries{init backend}} is guaranteed to stay mounted at {\ttfamily system\+:/elektra} where the configuration for Elektra itself is stored. After mounting all backends, Elektra checks if {\ttfamily system\+:/elektra} still resides at the default backend. If not, the init backend will be mounted there.\hypertarget{doc_help_elektra-bootstrapping_md_autotoc_md2765}{}\doxysection{SUMMARY}\label{doc_help_elektra-bootstrapping_md_autotoc_md2765}
...
The default backend consists of a default storage plugin and default resolver plugin. The default resolver has no specific requirements, but the default storage plugin must be able to handle full Elektra semantics. The backend is mounted to root ({\ttfamily /}), so any keys can be stored in it. The implementation of the core guarantees that user and system keys always stay separated.\hypertarget{doc_help_elektra-bootstrapping_md_autotoc_md2766}{}\doxysection{TRACEABILITY}\label{doc_help_elektra-bootstrapping_md_autotoc_md2766}

\begin{DoxyItemize}
\item {\ttfamily elektra\+Open\+Bootstrap()} implements above algorithm
\item {\ttfamily backend\+Open\+Default()} opens the default backend
...
\end{DoxyItemize}\hypertarget{doc_help_elektra-bootstrapping_md_autotoc_md2767}{}\doxysection{SEE ALSO}\label{doc_help_elektra-bootstrapping_md_autotoc_md2767}

\begin{DoxyItemize}
\item \mbox{\hyperlink{doc_decisions_6_implemented_bootstrap_md}{bootstrap decision}} 
\end{DoxyItemize}
\label{doc_help_elektra-semantics_md_md_doc_help_elektra_semantics}%
\Hypertarget{doc_help_elektra-semantics_md_md_doc_help_elektra_semantics}%
 The use of arbitrary metadata has extensive effects in Elektra’s semantics. They become simpler and more suited to carry key value pairs. The semantics now gives us independence of the underlying file system. So none of the file system\textquotesingle{}s restrictions apply anymore. No constraints on the length of a key name disturbs the user anymore. Additionally, key names can be arbitrarily deep nested. Depth is the number of unescaped {\ttfamily /} in the key name.
...
Hidden keys are not useful for Elektra. Instead comments or other metadata contain information about keys that is not considered belonging to the configuration. If hidden keys are desired, we can still write a plugin to filter specific keys out.\hypertarget{doc_help_elektra-semantics_md_autotoc_md2802}{}\doxysection{Problems}\label{doc_help_elektra-semantics_md_autotoc_md2802}
This section explains why using file system semantics for configuration is not a good idea.\hypertarget{doc_help_elektra-semantics_md_autotoc_md2803}{}\doxysubsection{filesys}\label{doc_help_elektra-semantics_md_autotoc_md2803}
...
If the backend {\ttfamily filesys} was the ideal solution, Elektra’s API (application programming interface) would be of limited use. E.\+g.\+cascading, type checking and optional cross-\/cutting features would be missing. The storage problem itself and the location of a key in a key database would be solved. because well-\/established APIs for accessing files are available in every applicable programming language.

Elektra 0.\+7 already supported more than one backend, but {\ttfamily filesys} was the only backend implementing the full semantics.\hypertarget{doc_help_elektra-semantics_md_autotoc_md2804}{}\doxysubsection{Limitations of File Systems}\label{doc_help_elektra-semantics_md_autotoc_md2804}

...
\Hypertarget{doc_tutorials_java-kdb_md_md_doc_tutorials_java_kdb}%
 \hypertarget{doc_tutorials_java-kdb_md_autotoc_md3625}{}\doxysection{Introduction}\label{doc_tutorials_java-kdb_md_autotoc_md3625}
When programming in Java it is possible to access the key database, changing values of existing keys or adding new ones and a few other things. It is also possible to write plugins for Elektra in Java but we will focus on using the Java binding in this tutorial.\hypertarget{doc_tutorials_java-kdb_md_autotoc_md3626}{}\doxysection{First Steps}\label{doc_tutorials_java-kdb_md_autotoc_md3626}
...
If an error occurs, detailed information can be obtained from the thrown {\ttfamily KDBException}.\hypertarget{doc_tutorials_java-kdb_md_autotoc_md3627}{}\doxysection{A word about releasing native ressources}\label{doc_tutorials_java-kdb_md_autotoc_md3627}
...
Fortunately the only resource you are strictly required to release is {\ttfamily KDB} via either {\ttfamily KDB\+::close} or {\ttfamily try-\/with-\/resouces}. For {\ttfamily Key\+Set} and {\ttfamily Key} the garbage collector cleans them up automatically using a \href{https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/lang/ref/Cleaner.html}{\texttt{ {\ttfamily Cleaner}}} In situations where memory is very constrained, it might be a good idea to clean up these native resources as soon as they are no longer needed, since we do not have any control of when or if the garbage collector cleans up after us. For this use-\/case, the following functions are provided\+: {\ttfamily Key\+Set\+::release}, {\ttfamily Key\+::release} and {\ttfamily KDBException\+::release\+Error\+Key}\`{}.\hypertarget{doc_tutorials_java-kdb_md_autotoc_md3628}{}\doxysection{Fetching keys}\label{doc_tutorials_java-kdb_md_autotoc_md3628}
...
\end{DoxyCode}
\hypertarget{doc_tutorials_java-kdb_md_autotoc_md3629}{}\doxysection{Saving Keys}\label{doc_tutorials_java-kdb_md_autotoc_md3629}
...
The {\itshape user} namespace is accessible without special rights, but if you try to write to {\itshape system} you will need to have root privileges. Take a look at \mbox{\hyperlink{doc_TESTING_md}{TESTING.md}} to see how to access the system namespace as non-\/root user. This should only be done in testing environments though as it is not intended for productive systems.\hypertarget{doc_tutorials_java-kdb_md_autotoc_md3630}{}\doxysection{Examples}\label{doc_tutorials_java-kdb_md_autotoc_md3630}
\hypertarget{doc_tutorials_java-kdb_md_autotoc_md3631}{}\doxysubsection{Traversing Keys in a $<$tt$>$\+Key\+Set$<$/tt$>$}\label{doc_tutorials_java-kdb_md_autotoc_md3631}
First we create a new {\ttfamily KDB} handle and fetch all keys for the desired namespace, in this example the whole {\ttfamily user} namespace. Since all all keys are put in the passed {\ttfamily key\+Set} variable we can then iterate through it. The {\ttfamily at(int)} method returns a new {\ttfamily Key} object for the native key with the corresponding position within the {\ttfamily key\+Set}.
...
\end{DoxyCode}
\hypertarget{doc_tutorials_java-kdb_md_autotoc_md3632}{}\doxysubsection{Read Multiple Keys From KDB}\label{doc_tutorials_java-kdb_md_autotoc_md3632}
This example shows how to read multiple keys. It provides comments for further clarification. Further information can be found here.\hypertarget{doc_tutorials_java-kdb_md_autotoc_md3633}{}\doxysection{Java Plugin Tutorial}\label{doc_tutorials_java-kdb_md_autotoc_md3633}

...
Let us start with a motivating example first\+:\hypertarget{doc_tutorials_mount_md_autotoc_md2707}{}\doxysection{Mount the Lookup Table for Hostnames}\label{doc_tutorials_mount_md_autotoc_md2707}
...
Because of that only root can mount files.\hypertarget{doc_tutorials_mount_md_autotoc_md2708}{}\doxysection{Resolver}\label{doc_tutorials_mount_md_autotoc_md2708}
...
But resolvers are not the only plugins Elektra uses\+:\hypertarget{doc_tutorials_mount_md_autotoc_md2709}{}\doxysection{Plugins}\label{doc_tutorials_mount_md_autotoc_md2709}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\# create a directory for our demonstration}
\DoxyCodeLine{mkdir -\/p example \&\& cd \$\_}
\DoxyCodeLine{}
\DoxyCodeLine{\# this creates the .git/config file}
\DoxyCodeLine{git init}
\DoxyCodeLine{}
\DoxyCodeLine{\# mount git’s configuration into Elektra}
\DoxyCodeLine{sudo kdb mount /.git/config dir/git ini multiline=0}
\end{DoxyCode}


As git uses the {\ttfamily ini} format for its configuration we use the \mbox{\hyperlink{md_src_plugins_ini_README_src_plugins_ini_README_md}{ini plugin}}. You can pass parameters to plugins during the mount process. This is what we did with {\ttfamily multiline=0}. Git indents the entries in its configuration files and the default behavior of the {\ttfamily ini} plugin is to interpret these indented entries as values that span multiple lines. The passed parameter disables this behavior and makes the ini-\/plugin compatible with git configuration.

Now let us see how smoothly the ini plugin sets and gets the git configuration.


\begin{DoxyCode}{0}
\DoxyCodeLine{\# set a user name ...}
\DoxyCodeLine{git config user.name "Rob Banks"}
\DoxyCodeLine{}
\DoxyCodeLine{\# ... and read it with kdb}
\DoxyCodeLine{kdb get dir/git/user/name}
\DoxyCodeLine{\#> Rob Banks}
\DoxyCodeLine{}
\DoxyCodeLine{\# set a user email with kdb ...}
\DoxyCodeLine{kdb set dir/git/user/email "rob.banks@dot.com"}
\DoxyCodeLine{}
\DoxyCodeLine{\# and read it with git}
\DoxyCodeLine{git config -\/-\/get user.email}
\DoxyCodeLine{\#> rob.banks@dot.com}
\end{DoxyCode}
\hypertarget{doc_tutorials_mount_md_autotoc_md2710}{}\doxysubsubsection{Meta Data}\label{doc_tutorials_mount_md_autotoc_md2710}
Elektra is able to store \mbox{\hyperlink{doc_help_elektra-metadata_md}{metadata}} of keys, provided the format of the file that holds the configuration supports this feature. The ini plugin does support this feature, and so does the \mbox{\hyperlink{md_src_plugins_ni_README_src_plugins_ni_README_md}{ni}} and the \mbox{\hyperlink{md_src_plugins_dump_README_src_plugins_dump_README_md}{dump}} plugin among others.
...
Meta data comes in handy if we use other plugins, than just the ones that store and retrieve data. I chose the {\ttfamily ni} plugin for this demonstration, because it supports metadata and is human readable. So let us have a look at the \mbox{\hyperlink{md_src_plugins_type_README_src_plugins_type_README_md}{type}} and \mbox{\hyperlink{md_src_plugins_mathcheck_README_src_plugins_mathcheck_README_md}{mathcheck}} plugins.


\begin{DoxyCode}{0}
\DoxyCodeLine{\# mount the backend with the plugins ...}
\DoxyCodeLine{kdb mount example.ni user/example ni type}
\DoxyCodeLine{}
...
If you want to find out more about validation I recommend reading \mbox{\hyperlink{doc_tutorials_validation_md}{this}} tutorial next.\hypertarget{doc_tutorials_mount_md_autotoc_md2711}{}\doxysubsubsection{Backends}\label{doc_tutorials_mount_md_autotoc_md2711}
The plugins together with the configuration file form a {\itshape backend}. The backend determines how Elektra stores data below a mount point. You can examine every mount points backend by looking at the configuration below {\ttfamily system/elektra/mountpoints/$<$mount point$>$/}.\hypertarget{doc_tutorials_mount_md_autotoc_md2712}{}\doxysection{Limitations}\label{doc_tutorials_mount_md_autotoc_md2712}

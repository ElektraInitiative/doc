...
\hypertarget{md_src_plugins_base64_README_autotoc_md70}{}\doxysubsection{Introduction}\label{md_src_plugins_base64_README_autotoc_md70}
...
The {\ttfamily base64} plugin encodes binary values before {\ttfamily kdb set} writes the configuration to the file. The values are decoded back to their original value after {\ttfamily kdb get} has read from the configuration file.\hypertarget{md_src_plugins_base64_README_autotoc_md71}{}\doxysubsection{Usage}\label{md_src_plugins_base64_README_autotoc_md71}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_base64_README_autotoc_md72}{}\doxysubsection{Modes}\label{md_src_plugins_base64_README_autotoc_md72}
...
. By default the plugin uses escaping mode which has the advantage that a storage plugin does not have to change its behavior at all to work in conjunction with Base64.\hypertarget{md_src_plugins_base64_README_autotoc_md73}{}\doxysubsubsection{Escaping Mode}\label{md_src_plugins_base64_README_autotoc_md73}
In order to identify the base64 encoded content, the values are marked with the prefix {\ttfamily @BASE64}. To distinguish between the {\ttfamily @} as character and {\ttfamily @} as Base64 marker, all strings starting with {\ttfamily @} will be modified so that they begin with {\ttfamily @@}.\hypertarget{md_src_plugins_base64_README_autotoc_md74}{}\doxyparagraph{Example}\label{md_src_plugins_base64_README_autotoc_md74}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_base64_README_autotoc_md75}{}\doxysubsubsection{Meta Mode}\label{md_src_plugins_base64_README_autotoc_md75}
...
\hypertarget{md_src_plugins_base64_README_autotoc_md76}{}\doxyparagraph{Example}\label{md_src_plugins_base64_README_autotoc_md76}

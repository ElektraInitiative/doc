...
This plugin is a storage plugin for reading and writing TOML files. The plugin retains most of the file structure of a read TOML file such as comments, empty lines and TOML tables. It supports all kinds of TOML specific types and tables, including nested inline tables and multiline strings.\hypertarget{md_src_plugins_toml_README_autotoc_md525}{}\doxysection{Requirements}\label{md_src_plugins_toml_README_autotoc_md525}
The plugin needs Flex ($>$=2.\+6.\+2) and Bison ($>$=3) for parsing TOML files.\hypertarget{md_src_plugins_toml_README_autotoc_md526}{}\doxysection{Types}\label{md_src_plugins_toml_README_autotoc_md526}
\hypertarget{md_src_plugins_toml_README_autotoc_md527}{}\doxysubsection{Reading}\label{md_src_plugins_toml_README_autotoc_md527}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md528}{}\doxysubsection{Writing}\label{md_src_plugins_toml_README_autotoc_md528}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md529}{}\doxysection{Numbers}\label{md_src_plugins_toml_README_autotoc_md529}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md530}{}\doxysection{Strings}\label{md_src_plugins_toml_README_autotoc_md530}
The plugin can read any kind of TOML string\+: bare, basic, literal, basic multiline and literal multiline. However, it will write back all non-\/bare strings as basic strings or it\textquotesingle{}s multiline version. Therefore, any string set with {\ttfamily kdb set} must be treated as a basic string and possible escape sequences and special meanings of quotation characters must be taken care of.
...
The plugin supports all kinds of escape sequences used by TOML in basic and basic multiline strings, like {\ttfamily \textbackslash{}n}, {\ttfamily \textbackslash{}r}, {\ttfamily \textbackslash{}t} and even {\ttfamily \textbackslash{}u}/{\ttfamily \textbackslash{}U} for Unicode escape sequences. {\ttfamily \textbackslash{}t} is interpreted to be 4 spaces.\hypertarget{md_src_plugins_toml_README_autotoc_md531}{}\doxysection{Binary/\+NULL values}\label{md_src_plugins_toml_README_autotoc_md531}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md532}{}\doxysection{TOML specific structures}\label{md_src_plugins_toml_README_autotoc_md532}
TOML specific structures are represented by the metakey {\ttfamily tomltype} on a certain key. It will be set when the TOML plugin reads a TOML structure from a file. Additionally, this metakey can be set by the user, if they want a certain TOML structure to be written. No automatic inference of this metakey is done on writing.\hypertarget{md_src_plugins_toml_README_autotoc_md533}{}\doxysubsection{Simple Tables}\label{md_src_plugins_toml_README_autotoc_md533}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md534}{}\doxysubsection{Table Arrays}\label{md_src_plugins_toml_README_autotoc_md534}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md535}{}\doxysubsection{Inline Tables}\label{md_src_plugins_toml_README_autotoc_md535}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md536}{}\doxysubsection{Arrays}\label{md_src_plugins_toml_README_autotoc_md536}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md537}{}\doxysection{Comments and Empty Lines}\label{md_src_plugins_toml_README_autotoc_md537}
...
Spaces can be added to a comment by creating a {\ttfamily comment/\#n/space} metakey with the amount of spaces to the key.
...
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/key' 'comment/\#0/space' '4'}
\DoxyCodeLine{}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md538}{}\doxysubsection{Comments in Arrays}\label{md_src_plugins_toml_README_autotoc_md538}
...
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array' 'comment/\#0/space' '5'}
\DoxyCodeLine{}
...
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#0' 'comment/\#0/space' '4'}
\DoxyCodeLine{}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#0' 'comment/\#1' ' Comment preceding the first element'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#0' 'comment/\#1/start' '\#'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#0' 'comment/\#1/space' '4'}
\DoxyCodeLine{}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#0' 'comment/\#2' ' Another comment preceding the first element'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#0' 'comment/\#2/start' '\#'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#0' 'comment/\#2/space' '6'}
\DoxyCodeLine{}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#1' 'comment/\#0' ' Inline comment of second element'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#1' 'comment/\#0/start' '\#'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#1' 'comment/\#0/space' '4'}
\DoxyCodeLine{}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#1' 'comment/\#1' ' Comment preceding the second element'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#1' 'comment/\#1/start' '\#'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#1' 'comment/\#1/space' '6'}
\DoxyCodeLine{}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#2' 'comment/\#0' ' Inline comment of the last element'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#2' 'comment/\#0/start' '\#'}
\DoxyCodeLine{kdb meta-\/set 'user:/tests/storage/array/\#2' 'comment/\#0/space' '5'}
\DoxyCodeLine{}
\DoxyCodeLine{\# Print the content of the resulting TOML file}
\DoxyCodeLine{cat `kdb file user:/tests/storage`}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_toml_README_autotoc_md539}{}\doxysection{Order}\label{md_src_plugins_toml_README_autotoc_md539}
...
In this example, {\ttfamily d} and {\ttfamily common} have the same parent, the file root. This means, they need to be sorted with each other. {\ttfamily d} would be placed before {\ttfamily common} by it\textquotesingle{}s order, since it was set before, and thus, has lesser order. However, their order never gets compared, since {\ttfamily common} is a simple table and {\ttfamily d} is not, so {\ttfamily d} will get sorted before the table regardless of order.\hypertarget{md_src_plugins_toml_README_autotoc_md540}{}\doxysection{Limitations}\label{md_src_plugins_toml_README_autotoc_md540}

...
\Hypertarget{doc_tutorials_plugins_md_md_doc_tutorials_plugins}%
 This file serves as a tutorial on how to write a plugin.\hypertarget{doc_tutorials_plugins_md_autotoc_md3924}{}\doxysection{Types of Plugins}\label{doc_tutorials_plugins_md_autotoc_md3924}
...
This tutorial mostly uses storage plugins as example but also explains differences to filter plugins.\hypertarget{doc_tutorials_plugins_md_autotoc_md3925}{}\doxysection{Basics}\label{doc_tutorials_plugins_md_autotoc_md3925}
First, there are a few basic points to understand about Elektra plugins. This first section will explain the basic layout of a plugin and what various methods exists within one.\hypertarget{doc_tutorials_plugins_md_autotoc_md3926}{}\doxysubsection{The Interface}\label{doc_tutorials_plugins_md_autotoc_md3926}
...
After these two steps your plugin is ready to be compiled, installed and mounted for the first time. Have a look at \mbox{\hyperlink{doc_tutorials_mount_md}{How-\/\+To\+: kdb mount}}\hypertarget{doc_tutorials_plugins_md_autotoc_md3927}{}\doxysubsubsection{C++ Based Plugins}\label{doc_tutorials_plugins_md_autotoc_md3927}
...
\end{DoxyCode}
\hypertarget{doc_tutorials_plugins_md_autotoc_md3928}{}\doxysection{Contract}\label{doc_tutorials_plugins_md_autotoc_md3928}
...
To avoid this situation, plugins export a so-\/called {\itshape contract}. In this contract the plugin states how nicely it will behave and what other plugins can depend on.\hypertarget{doc_tutorials_plugins_md_autotoc_md3929}{}\doxysubsection{Writing a Contract}\label{doc_tutorials_plugins_md_autotoc_md3929}
...
\end{DoxyItemize}\hypertarget{doc_tutorials_plugins_md_autotoc_md3930}{}\doxysubsection{Content of $<$tt$>$\+README.\+md$<$/tt$>$}\label{doc_tutorials_plugins_md_autotoc_md3930}
...
\end{DoxyCode}
\hypertarget{doc_tutorials_plugins_md_autotoc_md3931}{}\doxysection{Including $<$tt$>$readme\+\_\+pluginname.\+c$<$/tt$>$}\label{doc_tutorials_plugins_md_autotoc_md3931}
...
\end{DoxyCode}
\hypertarget{doc_tutorials_plugins_md_autotoc_md3932}{}\doxysection{CMake}\label{doc_tutorials_plugins_md_autotoc_md3932}
...
If your plugin makes use of \mbox{\hyperlink{doc_tutorials_compilation-variants_md}{compilation variants}} you should also read the information there.\hypertarget{doc_tutorials_plugins_md_autotoc_md3933}{}\doxysection{Coding}\label{doc_tutorials_plugins_md_autotoc_md3933}
This section will focus on an overview of the kind of code you would use to develop a plugin. It gives examples from real plugins and should serve as a rough guide on how to write a storage plugin that can read and write configuration data into an Elektra {\ttfamily Key\+Set}.\hypertarget{doc_tutorials_plugins_md_autotoc_md3934}{}\doxysubsection{$<$tt$>$elektra\+Plugin\+Get$<$/tt$>$}\label{doc_tutorials_plugins_md_autotoc_md3934}
...
For further information see \href{https://doc.libelektra.org/api/latest/html/group__plugin.html}{\texttt{ the API documentation}}.\hypertarget{doc_tutorials_plugins_md_autotoc_md3940}{}\doxysubsection{$<$tt$>$elektra\+Plugin\+Get\+Global\+Key\+Set$<$/tt$>$}\label{doc_tutorials_plugins_md_autotoc_md3940}
...
To improve performance, the {\ttfamily cache} plugin also caches parts of the global keyset. If your plugin uses non-\/cacheable data, you don\textquotesingle{}t have to do anything special. However, if you want your plugin\textquotesingle{}s keys to be cached you should put them below {\ttfamily system\+:/elektra/cached} (to avoid collisions, use e.\+g. {\ttfamily system\+:/elektra/cached/$<$plugin$>$} for {\ttfamily $<$plugin$>$}). The {\ttfamily cache} plugin also caches keys below {\ttfamily system\+:/elektra/cache}, but those are reserved for use by the plugin itself.\hypertarget{doc_tutorials_plugins_md_autotoc_md3941}{}\doxysection{Note on Direct Method Calls via External Integrations}\label{doc_tutorials_plugins_md_autotoc_md3941}
Some applications want to call Elektra methods directly via native access. A {\ttfamily Key\+Set} is a data structure over which functions can iterate. If you want to start again from to first element, you have to explicitly call {\ttfamily rewind()} to set the internal pointer to the start. Any plugin expects the passed {\ttfamily Key\+Set} to be {\bfseries{rewinded}}.\hypertarget{doc_tutorials_plugins_md_autotoc_md3942}{}\doxysection{Further Readings}\label{doc_tutorials_plugins_md_autotoc_md3942}
Read more about\+:

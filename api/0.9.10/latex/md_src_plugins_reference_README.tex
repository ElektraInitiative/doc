...
If a key has metakey {\ttfamily check/reference} with value {\ttfamily recursive}, the plugin constructs a reference graph starting with the marked key. (for the exact construction see below) In this reference graph each node corresponds to a key in the KDB, while each edge represents a reference between to keys. The plugin will produce an error, if this graph is not a directed acyclic graph or contains any invalid references.\hypertarget{md_src_plugins_reference_README_autotoc_md466}{}\doxysection{Installation}\label{md_src_plugins_reference_README_autotoc_md466}
See \mbox{\hyperlink{doc_INSTALL_md}{installation}}. The package is called {\ttfamily libelektra5-\/experimental}.\hypertarget{md_src_plugins_reference_README_autotoc_md467}{}\doxysubsection{Resolution of References}\label{md_src_plugins_reference_README_autotoc_md467}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_src_plugins_reference_README_autotoc_md468}{}\doxysubsection{Construction of the Reference Graph}\label{md_src_plugins_reference_README_autotoc_md468}
...
Once the whole process is finished, we will have a graph of the whole reference structure stemming from {\ttfamily X}. This graph can then be check for acyclicity.\hypertarget{md_src_plugins_reference_README_autotoc_md469}{}\doxysubsection{Restriction of References}\label{md_src_plugins_reference_README_autotoc_md469}
...
Note\+: If the restriction is the empty string {\ttfamily \char`\"{}\char`\"{}} {\bfseries{no}} keyname will match, meaning the reference key annotated with this metadata has to be a leaf node in the reference graph, and therefore cannot have a value (other than the empty string {\ttfamily \char`\"{}\char`\"{}}).\hypertarget{md_src_plugins_reference_README_autotoc_md470}{}\doxysection{Usage}\label{md_src_plugins_reference_README_autotoc_md470}
...
\end{DoxyCode}
\hypertarget{md_src_plugins_reference_README_autotoc_md471}{}\doxysection{Examples}\label{md_src_plugins_reference_README_autotoc_md471}
